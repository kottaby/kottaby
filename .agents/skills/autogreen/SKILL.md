...
name: autogreen
description: >
  End-to-end autonomous green loop for this repository. Detects every failure across the full
...
> **Not to be confused with `autofix`** — that skill applies CodeRabbit PR review-thread feedback.
> Use `autogreen` to make the whole pipeline green; use `autofix` to resolve PR review comments.
