...
name: skill-creator
description: >
  Guide for creating and validating effective skills for this project and agent workflows. Use this skill when:
  (1) users want to create a new skill,
  (2) users want to update or customize an existing skill,
  (3) users ask about skill structure or best practices,
  (4) users need help writing SKILL.md files and validating them.
license: MIT
...
# Skill Creator & Validator Guide

This guide helps you create, customize, and validate effective skills following the [Agent Skills specification](https://agentskills.io/specification) and workspace standards.
...
After creating or modifying any skill in `.agents/skills/`, run the project validator script:
...
2. YAML frontmatter has valid `name` (matching directory name in kebab-case) and `description` (with triggers).
3. Markdown body has substantive guidance.

## Checklist for New Skills

Before considering a skill ready, verify:

- [ ] `name` matches directory name and follows naming rules (`^[a-z0-9-]+$`)
...
- [ ] Automated validation passes: `bun run scripts/validate-skills.ts`

## Ground Rules

- ALWAYS include trigger conditions in the description
- ALWAYS test skill validity with `bun run scripts/validate-skills.ts`
