...
The current release candidate is **`drizzle-orm@1.0.0-rc.4`** (the `@rc` tag), which supersedes the
...
## Kottaby Database & Drizzle Conventions (CRITICAL)

In this project, adhere strictly to these architectural rules:

1. **Schema Location**: Table definitions live in `backend/db/schema/`.
2. **Type Definition Pattern**:
   - Database and model types live strictly in `backend/types/{entity}.types.ts`.
   - Use `{Entity}SelectType` (inferred select) and `{Entity}InsertType` (inferred insert).
   - Use `DBTransaction` and `DBQueryExecutor` from `@/backend/types` for transaction typing.
   - Never create ad-hoc types or local type definitions in Pothos resolvers.
3. **Repository Pattern & Transaction Propagation**:
   - Repositories live in `backend/db/repo/<domain>/<name>.repository.ts`.
   - Every repository method performing queries/writes **MUST** accept an optional `tx?: DBTransaction` argument.
   - Inside transactions, pass `tx` to **all** repo calls: `await SomeRepository.update(id, data, tx)`.
4. **Database Testing**:
   - Database tests MUST run inside `runInRollback` from `@/backend/db/test/test-utils`.
   - Always pass `tx` to all repository methods in tests.
   - NEVER use `expect(...).rejects.toThrow()` inside `runInRollback` (causes transaction deadlocks); use `expectRepoError()` try/catch.
   - NEVER query seed data — use helpers in `backend/db/test/entity-setup.ts`.
5. **Database Actions**: Run `bun run db` for all migration and generation tasks.
