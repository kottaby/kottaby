...
| Status chip tone contradicts its lifecycle label (e.g. a FAILED chip success-toned because the branch news is positive) | Branch-specific tone chose the NEWS sentiment instead of the STATUS semantics | Chip stays in the lifecycle-status family (failed to warning/error); the positive news lives in the zone body copy, never the chip | Check ALL branches of the same status render the same chip family |
