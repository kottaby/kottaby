...
description: >
  ...
  with the correct milestone milestone, labels, assignee, dependency cross-links, and a plan-backed
  issue body. Use this skill when: (1) the user asks to open/create a GitHub issue for a ticket,
  milestone, or plan under ai/plans, (2) the user wants issues synced from docs/planning/TICKETS.md,
...
backed by its implementation plan under `ai/plans/milestone_N/<slug>/` when one exists.
...
- [ ] Extract the ticket entry from `docs/planning/TICKETS.md` (search `### \[<TICKET-ID>`); capture title, milestone, story points, blocked-by, description, acceptance criteria, test scenarios, decision refs.
- [ ] Cross-check dependencies against the milestone backlog table in `docs/planning/MILESTONE_PLAN.md` (the milestone plan may list dependencies not in the ticket entry).
...
- [ ] Locate the plan bundle: `ls ai/plans/milestone_<N>/<slug>/` (expect `plan.md`, `specs.md`, `tasks.md`, optional `deferred-items.md`, `outcome/`, `prototype/`). Read `specs.md` for the summary material.
...
- [ ] Resolve milestone: `gh api repos/<owner>/<repo>/milestones --jq '.[].title'`. Milestone 0 → `Foundation`, Milestone 1 → `Core Domain MVP`. For milestones without an existing milestone, create one named per `docs/planning/MILESTONE_PLAN.md` and tell the user.
- [ ] Ensure labels exist: `milestone-<N>` (create it if missing, e.g. `gh label create milestone-2 --color 0052CC`) and `enhancement`. NEVER create or apply `dev-1`/`dev-2`/`dev-3` stream labels.
- [ ] Compose the issue body per the template below.
- [ ] Create the issue: `gh issue create -R <owner>/<repo> --title "..." --assignee <login> --milestone "<Milestone>" --label milestone-<N>,enhancement --body "$(cat <<'EOF' ... EOF)"` (quoted heredoc delimiter so backticks in Gherkin/code survive).
...
Full plan bundle under [`ai/plans/milestone_<N>/<slug>/`](<GitHub tree URL on main>):
