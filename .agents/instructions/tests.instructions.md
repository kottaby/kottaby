---
applyTo: "**/*.test.ts,**/*.test.tsx,**/*.spec.ts,**/*.spec.tsx,test/scripts/**/*.ts"
---
...
- `tx` position VARIES per repo method - check the method signature/source before calling, never assume position

### Database Tests - Entity Setup Helpers

- Helper signatures VARY - read source in `entity-setup.ts` before calling; create entities in dependency order (parent IDs come from previously created rows)
- Never assume schema relations or table columns - read the schema in `backend/db/schema/` first
...
- Assert `.toContain()` on translated message substring - NOT raw translation key
- Repository methods produce localized messages - raw keys never appear in `error.message`
...
- PGlite permits exactly ONE open per data dir — serialize DB commands (`generate`/`migrate`/`push`/`seed`) and DB-backed suites through the process-lock runners, and NEVER run the live-server (`setupTestServerLifecycle`) harness concurrently with DB-backed suites in the same worktree (concurrent opens cause WAL corruption).
...
- Run with log capture: `bun run test/scripts/run-test.ts <test-path>` (not raw `bun test`)
- View last result: `bun run test/scripts/run-test.ts --last <path>`
- Filtered view: `bun run test/scripts/run-test.ts --last --focus "<pattern>" <path>`
- AI agents MUST use this script instead of raw `bun test` for database tests

### Run-Test Script Modification Rules

When modifying `test/scripts/run-test.ts`:
...
- Update `--help` output and argument parsing for any new flags
- No external dependencies - script must remain self-contained
- Test changes end-to-end with a small smoke test invocation
- Run quality: `bun tsgo` and lint after changes

### GraphQL Integration Tests (`frontend/graphql/test/`)
...
- Null guard before assertions on query results:
  ...
  if (!items) throw new Error("items should be defined");
  ```
...
- Look up mutation variable types in `frontend/graphql/generated/` (search for `{MutationName}MutationVariables`)
...
- Use valid enum values from generated types (e.g., `CurrencyCode.Usd`) — never hardcode strings
