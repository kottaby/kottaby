...
## Phase Navigation
- **Previous:** [Design Phase](design-phase.md) - Must be completed first (Phase 2)
- **Plan Review:** Phase 1.5 - Invoke @plan-review skill on complete plan BEFORE implementation
- **Current:** **Tasks Phase (Phase 3+)** - Break down design into actionable steps
- **Mid-Point:** Phase 2.5 (conditional) - Backend review checkpoint
- **Next:** [Implementation](../execution/implementation-guide.md) - Execute the tasks (Task 0 → Phase 1 → ...)
- **Context:** [Process Overview](README.md) - Development lifecycle
...
### Knowledge Propagation Tasks (MANDATORY Final Task)

**Purpose**: Propagate learnings from plan execution outcomes back into the project's permanent knowledge base
...
- Each domain has its own subdirectory: `docs/drizzle/`, `docs/auth/`, `docs/frontend/`, etc.
- If no matching subdirectory exists, create one: `mkdir -p docs/<domain>/`
- File naming: `kebab-case.md` (e.g., `prepared-statements.md`)
- Docs file structure: Why → Pattern → Rules → Anti-patterns → Rollout Summary → Related Documents
