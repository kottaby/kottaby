...
--              re-settled.
--            * OLD.type must be 'withdrawal' — only withdrawal rows settle;
--              an earning or bonus row can never be relabeled into a
--              settlement (or settled at all).
--            * NEW.type must be 'withdrawal' and unchanged from OLD.type —
--              only withdrawal rows carry a payout outcome; earning and
--              bonus rows have no lifecycle and stay frozen in every
--              direction.
