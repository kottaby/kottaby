...
- **Auth service: see `docs/auth/jwt-authentication-service.md` for the JWT auth contract (token claims, cookie matrix, redirect-loop fix, governance gate, `AuthService.login`/`refreshToken`, `assertUserActive`, RBAC consumption guide).**
...
- **Subscription purchase & activation: `SubscriptionPurchaseService` (purchase / listOwn) and `SubscriptionActivationService` (webhook settlement) own the billing purchase contract — idempotency claim → pending pair → signature-verified webhook → guarded activation (zero rows = replay; exactly-once credit) with a fail-closed NULL plan lane; never write `student_payments` outside the guarded decision writers, and acks replays `200 { processed: true, replayed: true }` and quarantines unknown/mismatched/unconfigurable events `200 { processed: false }` instead of throwing. See `docs/billing/subscription-purchase.md`.**
...
- **Platform analytics (admin read model): `PlatformAnalyticsService.getPlatformAnalytics` (`backend/services/admin/platform-analytics.service.ts`) composes fresh per-request aggregate snapshots — the database is read on EVERY request (never cached, never memoized), the happy path writes ZERO rows and emits ZERO audit/notification records, and the governed-reader re-check runs at the service tier via the shared admin gate (`assertActorAdminActive` as the FIRST statement). See `docs/admin/platform-analytics.md` for the metric contracts, money-as-string rules, and read-purity/no-audit rule.**
...
- **Session lifecycle: `SessionLifecycleService` (`backend/services/classes/session-lifecycle.service.ts`) is the sole owner of the session state machine — every transition is a guarded single-statement UPDATE (zero-row misses classified by a cold probe read that never feeds a write); "hold" = a guarded debit of one allowance unit at request (trial lane attempted first) with a same-lane refund on cancel; idempotent creation via the `session_request_idempotency` claim table (replays THROW `DUPLICATE_REQUEST` — the client maps the 409 to a success-equivalent notice); the service writes ZERO audit/report rows and NEVER writes a notification row directly — lifecycle session flows compose notification waves through the notification service's receipt contract (rows written exclusively by the `NotificationEngine` inside the owning transaction, receipts published strictly post-commit), and its ONLY direct cross-surface write is the wallet repository's credit of the teacher's earnings when a completed session is confirmed. See `docs/sessions/session-lifecycle.md`.**
- **Admin session governance: the six admin-only session operations (directory list/detail + reschedule/cancel/reassign/join) live in `SessionAdminGovernanceService` (`backend/services/classes/session-admin-governance.ts` + `session-admin-governance.helpers.ts`): gate (`assertAdminGovernanceClean`) → boundary schema → guarded UPDATE → refund/audit/claim in ONE tx → waves post-commit; the cancel reason is length-capped AND control-character-free so the serialized audit envelope always fits. See `docs/admin/admin-session-governance.md`.**
- **Session report & homework: `SessionReportService` (`backend/services/classes/session-report.service.ts`) is the sole owner of the post-completion academic record — one write gate (teacher role + service-tier governance re-check + session ownership + `status=completed`, classified under a FOR UPDATE row lock), one transaction co-creating report + optional homework (a homework row without its report is unreachable), and one report per session (`reports_session_id_unique` → typed `SESSION_REPORT_ALREADY_EXISTS`; `previousGrades` grade via the one-shot guarded `gradeHomeWorkOnce`). Reads are participant-only with an oracle-collapse `null`. Emissions ride the caller's tx via `SessionReportNotificationService.notifySessionReportReady` and publish strictly after commit — the single-writer `NotificationEngine` discipline (never insert `notifications` rows from domain services) applies unchanged. See `docs/sessions/session-report-homework.md`.**
