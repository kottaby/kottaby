...
- **Cold-start certification (single writer)**: `ColdStartCertificationService.certifyTeacherColdStart` (`backend/services/admin/cold-start-certification.service.ts`) is the single writer of the certified-state (`is_approved=true`) path outside the evaluation loop — governed by `docs/admin/cold-start-certification.md`.
...
- **Parent-link workflow**: `ParentLinkRequestService` (`backend/services/parents/parent-link-request.service.ts`) owns the parent→student link requests — `students.parent_id` is written ONLY via the guarded `StudentRepository.linkParentIfUnlinked` on student confirmation (zero-row ⇒ throw ⇒ rollback), expiry is strict-`>` and lazily materialized (reads never write), and cancel/expiry emit ZERO notifications. See `docs/parents/parent-link-request.md`.
