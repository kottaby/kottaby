...
/**
 * The session-scoped audit trail read's page size: the trail service's
 * per-page ceiling. A session's arbitration history is bounded by its own
 * lifecycle, so the single largest page IS the full session-scoped trail
 * for this surface (deeper pagination is the trail browse surface's job).
 */
