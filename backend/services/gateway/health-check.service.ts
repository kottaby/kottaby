 ...
 * sanctioned gateway probes (the `_health` GraphQL surface and the
 * `/api/health` HTTP probe).
 *
 ...
 *     an explicit future extension point, NOT this service.
 ...
 *     (`status`, `service`, `version`, `timestamp`) — exempt from ar/en
 *     locale parity (probe consumers are load balancers / CI smoke checks,
 *     never end users).
 */
