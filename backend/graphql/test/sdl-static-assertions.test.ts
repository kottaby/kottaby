 ...
 *    35-op baseline (the prior 7-op auth-quartet + notification read-latch
 ...
 *    purchase write + the session-report write + the student-evaluation
 *    rating write) and the Query root is
 *    EXACTLY the refreshed 34-op baseline (the prior frozen baseline +
 ...
 * session-report write + the student-evaluation rating write.
...
describe("BFLA structural verdict — zero notification CUD surface (REQ-032)", () => {
  ...
  test("Mutation root is EXACTLY the refreshed frozen 35-op baseline — the reconciled admin-user trio + quartet + dispute pair + confirm + payout + the sanctioned admin-governance pair + the session-governance quartet + the subscription purchase write + the session-report write + the student-evaluation rating write on top of the auth quartet + notification read-latch pair + users-locale surface", () => {
  ...
  test("Query root is EXACTLY the refreshed frozen 34-op baseline (the sanctioned student-evaluation caller-scoped read; zero unsanctioned growth)", () => {
