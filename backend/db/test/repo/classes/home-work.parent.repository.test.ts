/**
 * HomeWorkRepository.listForStudent / countForStudent tests — the
 ...
 *    NULLS LAST, home_work.id DESC` order; count matches list length on a
 *    known fixture; hit + miss (empty window past the end + honest total
 *    > 0); the row's projected column set is exactly the twelve
 *    `HomeWorkSelectType` fields.
 *  - Tier 2 (boundary): student with zero homework rows (count = 0, list
 ...
 *  - Tier 3 (chaos): cross-student isolation — a homework row whose
...
/** The exact key set of a `HomeWorkSelectType` row (alphabetical). */
...
const SORTED_HOME_WORK_ROW_KEYS: string[] = [...HOME_WORK_ROW_KEYS].toSorted(byKeyAscending);

/** Asserts set-equality of a row's own keys against the projected set. */
function expectExactRowKeys(row: HomeWorkSelectType): void {
  expect(Object.keys(row).toSorted(byKeyAscending)).toEqual(SORTED_HOME_WORK_ROW_KEYS);
}
 ...
 * and a homework row hanging off it.
 */
...
beforeAll(async () => {
  committed = await db.transaction(async tx => {
    ...
    // Promise.all over an async mapper — elides the `no-await-in-loop`
    // rule. Each mapper creates one session + one homework row.
    const sessionIds = await Promise.all(
      stamps.map(stamp =>
        (async () => {
          const sessionId = await createSessionWithStartedAt(tx, actors, stamp);
          await createTestHomeWork(tx, sessionId, jadidMadiOverrides());
          return sessionId;
        })()
      )
    );
    // One foreign-student session + homework — must NEVER surface in the
    ...
    await createTestHomeWork(tx, foreignSession.id, jadidMadiOverrides());
...
afterAll(async () => {
  ...
  // Hard delete in FK-dependency order: sessions cascade homework (the
  // home_work.session_id FK is ON DELETE CASCADE), then the role-child
  // rows, then the users rows (which cascade students/teacher rows).
  ...
  expect(await HomeWorkRepository.countForStudent(fixture.studentUserId)).toBe(0);
});

describe("HomeWorkRepository.listForStudent / countForStudent — transactional path (runInRollback)", () => {
  ...
  test("list returns rows in session.started_at DESC NULLS LAST, id DESC order; count matches list length", async () => {
    await runInRollback(async tx => {
      ...
      await createTestHomeWork(tx, oldestId, jadidMadiOverrides());
      await createTestHomeWork(tx, newestId, jadidMadiOverrides());
      await createTestHomeWork(tx, nullStartedAtId, jadidMadiOverrides());

      const rows = await HomeWorkRepository.listForStudent(actors.studentUserId, 50, 0, tx);
      const count = await HomeWorkRepository.countForStudent(actors.studentUserId, tx);
      expect(count).toBe(3);
      ...
      // The projected column set is exactly the twelve fields — no
      // `SELECT *` drift.
  ...
  test("list + count share the same filtered set (predicate cohesion) across pages", async () => {
    await runInRollback(async tx => {
      const actors = await createSessionActors(tx);
      const now = Date.now();
      // Promise.all over an async mapper — five sessions + homework, ids
      ...
      const total = await HomeWorkRepository.countForStudent(actors.studentUserId, tx);
      expect(total).toBe(5);

      // Page 1: limit 2, offset 0 → first two rows (newest two).
      const page1 = await HomeWorkRepository.listForStudent(actors.studentUserId, 2, 0, tx);
      ...
      const page2 = await HomeWorkRepository.listForStudent(actors.studentUserId, 2, 2, tx);
      ...
      const page3 = await HomeWorkRepository.listForStudent(actors.studentUserId, 2, 4, tx);
  ...
  test("the Jadid/Madi track blocks survive the projection verbatim (no fabricated zeros)", async () => {
    await runInRollback(async tx => {
      const actors = await createSessionActors(tx);
      const sessionId = await createSessionWithStartedAt(tx, actors, new Date());
      // Insert a homework row with BOTH tracks fully null (an
      // assignment-without-grades state). The list must preserve every
      // nullable column as `null` — never coerce to `0` or a default.
      await createTestHomeWork(tx, sessionId);
      ...
      expect(row.currentSurahJuz).toBeNull();
      expect(row.revisionSurahJuz).toBeNull();
      expect(row.currentFromAyah).toBeNull();
      expect(row.currentToAyah).toBeNull();
      expect(row.revisionFromAyah).toBeNull();
      expect(row.revisionToAyah).toBeNull();
    });
  });

  // ─── Tier 2: boundary ────────────────────────────────────────────────

  test("student with zero homework rows → list is [] and count is 0 (empty window)", async () => {
    await runInRollback(async tx => {
      const actors = await createSessionActors(tx);
      const rows = await HomeWorkRepository.listForStudent(actors.studentUserId, 50, 0, tx);
      const count = await HomeWorkRepository.countForStudent(actors.studentUserId, tx);
  ...
  test("offset beyond the end of the filtered set yields [] next to the honest total > 0", async () => {
    await runInRollback(async tx => {
      ...
      const rows = await HomeWorkRepository.listForStudent(actors.studentUserId, 10, 100, tx);
      expect(rows).toEqual([]);
    });
  });
  ...
  test("same-instant startedAt rows are deterministic by the id DESC tiebreak across pages", async () => {
    await runInRollback(async tx => {
      ...
      await createTestHomeWork(tx, sessionA, jadidMadiOverrides());
      await createTestHomeWork(tx, sessionB, jadidMadiOverrides());
      await createTestHomeWork(tx, sessionC, jadidMadiOverrides());
  ...
  test("cross-student isolation: a homework row whose session belongs to another student never surfaces", async () => {
    await runInRollback(async tx => {
      const actorsA = await createSessionActors(tx);
      const actorsB = await createSessionActors(tx);
      // Foreign student's session + homework (visible only to student B).
      const foreignSessionId = await createSessionWithStartedAt(tx, actorsB, new Date());
      await createTestHomeWork(tx, foreignSessionId, jadidMadiOverrides());
      // Primary student has zero homework — the foreign homework must not
      // leak into the primary student's reads.
      const rowsA = await HomeWorkRepository.listForStudent(actorsA.studentUserId, 50, 0, tx);
      const countA = await HomeWorkRepository.countForStudent(actorsA.studentUserId, tx);
      expect(countA).toBe(0);
      expect(rowsA).toEqual([]);
      // Foreign student sees exactly one row.
      const rowsB = await HomeWorkRepository.listForStudent(actorsB.studentUserId, 50, 0, tx);
      const countB = await HomeWorkRepository.countForStudent(actorsB.studentUserId, tx);
  ...
  test("homework written inside runInRollback vanishes after the forced rollback", async () => {
    ...
    await runInRollback(async tx => {
      ...
      expect(await HomeWorkRepository.countForStudent(rollbackStudentId, tx)).toBe(1);
    });
    // …and invisible on a fresh session AFTER the forced rollback.
    expect(await HomeWorkRepository.countForStudent(rollbackStudentId)).toBe(0);
  });
});

describe("HomeWorkRepository.listForStudent / countForStudent — standalone executor path (committed fixture)", () => {
  test("count runs on the queryDb read path and returns the committed fixture's honest total", async () => {
    ...
    // 4 sessions + homework for the primary student (3 with non-NULL
    // startedAt, 1 with NULL). The foreign student's homework does NOT
    // inflate this count.
    const count = await HomeWorkRepository.countForStudent(fixture.studentUserId);
    expect(count).toBe(4);
  });

  test("list runs on the queryDb read path and returns rows in DESC NULLS LAST order", async () => {
    const fixture = requireCommitted();
    const rows = await HomeWorkRepository.listForStudent(fixture.studentUserId, 50, 0);
    expect(rows).toHaveLength(4);
    // The NULL-startedAt session pins last under NULLS LAST.
    const lastRow = rows[3];
    expect(lastRow?.sessionId).toBeDefined();
    // Every non-NULL-startedAt row sorts before the NULL one in DESC order.
    for (let i = 0; i < 3; i += 1) {
      expect(rows[i]?.sessionId).toBeDefined();
    }
    // Foreign student's homework never leaks into the standalone read.
    for (const row of rows) {
      expectExactRowKeys(row);
    }
  });
  ...
  test("list paginates on the standalone path — page 1 returns the first two rows", async () => {
    const fixture = requireCommitted();
    const page1 = await HomeWorkRepository.listForStudent(fixture.studentUserId, 2, 0);
    expect(page1).toHaveLength(2);
  ...
  test("count is 0 for an unknown student on the standalone path", async () => {
    expect(await HomeWorkRepository.countForStudent(2_000_000_000)).toBe(0);
  });

  test("cross-tenant: the foreign student's count does not include the primary student's homework", async () => {
    const fixture = requireCommitted();
    const foreignCount = await HomeWorkRepository.countForStudent(fixture.foreignStudentUserId);
