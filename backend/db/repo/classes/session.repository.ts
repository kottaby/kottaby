 ...
 *    cancel-state membership and every other predicate are composed from
 *    equality conditions. No SQL line-comment sequences in any statement.
...
export namespace SessionRepository {
   ...
   * cancellation. The held-fee refund to the funding lane is the caller's
   * follow-up write inside the same transaction, driven by the returned
   * row's provenance lane.
   *
   * @returns The updated row, or `null` when zero rows matched (unknown id,
   ...
   * + lifecycle state + both participants) for a session id. Used ONLY
   * AFTER a guarded transition matched zero rows, to distinguish an unknown
   * id from a non-participant caller from a wrong-state row; the probe
   * never gates or influences any write.
   *
   * @returns The four-column projection, or `null` when the id is unknown.
   */
