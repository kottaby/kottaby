...
├── billing/             billing
├── books/               book, report
├── classes/             session, report, home-work, session-request-idempotency repositories
...
- **`TeacherRepository` (governed write, teachers domain)**: `TeacherRepository` (`backend/db/repo/teachers/teacher.repository.ts`) hosts the **cold-start certification** writers — `insertColdStartCertified` + guarded `elevateToCertified` ONLY; no other cold-start certification writers. The normal passed-applicant `teacher`-row write belongs to the evaluation-aggregation flow (the second of the exactly two certification writers, B.7) — see `docs/teachers/applicant-lifecycle.md` §1/§6. See `docs/admin/cold-start-certification.md`.
...
- **Session report & homework repos**: `ReportRepository` / `HomeWorkRepository` (`backend/db/repo/classes/report.repository.ts`, `home-work.repository.ts`) own the post-completion academic record — inserts + `findBySessionId` reads, `findLatestByStudentId` (newest prior row), and the one-shot guarded `gradeHomeWorkOnce` (both grade columns still-NULL folded into the predicate, `RETURNING`); the zero-row miss classifies no-prior-row (valid no-op) vs already-graded (typed conflict) at the service tier. `SessionRepository` adds `lockForReportGate` (FOR UPDATE gate probe, tx REQUIRED) + `findReportWaveContextById` (one joined student/teacher/parent locale read). One-report/one-homework-per-session is arbitrated by the unique constraints `reports_session_id_unique` / `home_work_session_id_unique`; a PG 23505 on either maps service-side to the typed `SESSION_REPORT_ALREADY_EXISTS` conflict. See `docs/sessions/session-report-homework.md`.
...
- **Subscription purchase & settlement (billing)**: the purchase/payment-gateway domain's canonical contract — idempotent purchase transactions, credit-lane guarded writers (zero rows = replay), and the webhook security gates upstream of them. See `docs/billing/subscription-purchase.md`.
