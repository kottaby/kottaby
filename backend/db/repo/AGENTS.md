...
- **Guarded transition pattern (session lifecycle)**: a state-machine write is ONE `UPDATE … WHERE <row identity> AND <owner/participant> AND <required pre-state> RETURNING *` — predicate evaluation and mutation share one statement under the row lock (zero check-then-write window). A zero-row match is classified by a cold probe read that runs only AFTER the miss and NEVER feeds a write. See `docs/sessions/session-lifecycle.md` §2.
- **`session.held_balance_lane` provenance**: the lane that funded a hold (`trial|hifz|tajweed`) is written once at creation and never rewritten or nulled — release/consumption flips `fee_held` only; refunds (and the future DEV3-012/013 confirm/sweeper flows) read the row's recorded lane to return the unit to the SAME lane. Never "clean it up" on cancel/complete.
- **`FOR UPDATE` locking reads (certification-lock precedent)**: `TeacherRepository.lockForCertificationCheck` — a locking read MUST take a REQUIRED `tx` (a lock released at statement end protects nothing), never ride the `queryDb` non-transactional fast path, use a minimal projection, and return an honest `null` for a missing row. Note: `FOR UPDATE` cannot block on uncommitted INSERTs under READ COMMITTED — concurrency tests must lock COMMITTED rows.
