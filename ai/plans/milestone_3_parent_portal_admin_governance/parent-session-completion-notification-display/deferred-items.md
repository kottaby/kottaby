...
| D4 | `ParentHomeworkEntryReturnType.sessionId` presence verification — IF missing, add to the closed projection + mapper + `parent-monitoring.documents.ts` + codegen (R-G conditional branch) | 7 (planning research) | 8 | ✅ Done | Task 8 outcome (8.1-tab-threading-outcome.md): sessionId VERIFIED PRESENT end-to-end — type :129, mapper emits, document selects, generated type; zero projection change owed | "Verify at implementation" ruling from research-00 §3 R-G; resolves inside Task 8's tab-threading work (REQ-013/REQ-014). If already present, mark ✅ with the verification reference. |
...
| D6 | `docs/parents/monitoring-portal.md` DEV1-017 display-contract section update (knowledge propagation) | 3 (field + link contract established) | 10 | ✅ Done | Task 10 outcome (10-knowledge-propagation-outcome.md): shipped-contract section landed (:314-369), R16 contract extended, count-drift reconciled, 14/14 links resolve | Extends the binding deep-link contract at `docs/parents/monitoring-portal.md:207` and the DEV1-017 forward item `:307` with the resolver/portal display behavior. REQ-061. |
