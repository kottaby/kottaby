...
| DI-0.2-02 | Dashboard-home slot component-test suite does not exist: `test/ui/components/dashboard/` contains only `profile-view.test.tsx`; there is no `RoleDashboardPage`/status-slot test. Tasks 4.3.TE / 5.1 assumed "extending" existing dashboard-home tests. | 0.2 | 4.3 (5.1 battery) | ✅ Done | 0.2-outcome.md §1 additional-checks table; 4.3-outcome.md §2 (file table) + §3; 5.1-outcome.md §7 | Resolved by 4.3: test/ui/components/dashboard/RoleDashboardPage.slot.test.tsx created — FIRST `RoleDashboardPage`/slot suite (7 pass / 0 fail / 51 expect; scoped dashboard/+students/ run 73 pass / 0 fail); re-confirmed green at the 5.1 battery (7/51). |
...
| DI-2.1-01 | Layer-wide `bun run test/scripts/run-test.ts test/workflows` is NOT green: 13 pre-existing failures in OTHER domains' journeys — `admin/account-governance.journey.test.ts` (step 8 login byte-identity `toEqual(users row)` / step 9 denial battery), `sessions/session-lifecycle*.journey.test.ts` (sub-second deadline-window drift `86399595 < 86400000`, `SESSION_NOT_FOUND` vs `VALIDATION` code mismatch, PGlite `execProtocolRawSync` driver error), `notifications/j1-targeted-single-recipient.test.ts` (step 9 catch-up `createdAt` precision `.000Z` vs `.898Z`). | 2.1 | 2.M (reclassified at 2.M), 5.1 final gate, + owning session/governance/notification tickets | 📅 Forward | 2.1-outcome.md §5 (isolation proof); 2.M-outcome.md §1 | **Reclassified from Blocked to Forward at the 2.M gate (2026-09-07).** Pre-existing on origin/base, outside plan scope, surfaced to user in Execution Summary; not attributable to DEV1-015 delta (zero production diff in those domains). Proof: each of the 13 failures reproduces in ISOLATION with the DEV1-015 files absent (2.1-outcome.md §5), and the 2.M gate diff audit shows ZERO edits under `backend/db/**`, `backend/graphql/**`, `backend/services/**` (non-test), `frontend/**`, `app/**`, `sessions/**`, `notifications/**`, `admin/**`. 2.M gate scope per tasks.md 2.M = `backend/services/parents` + `test/workflows` (parents domain) — both fully green (96 pass/15 skip/0 fail and 38 pass/1 skip/0 fail). Layer-wide greenness belongs to the 5.1 final gate + the owning foreign-domain tickets, which must also NOT touch those domains (scope gate). |
| DI-3.1-01 | `backend/graphql/test/schema-surface.test.ts` reports 4 pre-existing pin-drift cells (root-query freeze, mutation-set freeze, enum-set pin, whole-schema type-delta pin) driven by surfaces landed by OTHER features — `adminBroadcastNotification`, `adminCertifyTeacherColdStart`, `cancelParentLinkRequest`, enums `BroadcastAudienceType`/`LinkStatus`. Counts (37 pass / 4 fail / 262 expect / 41 tests) are byte-identical at 3.1 and 5.1; the file itself is unmodified in this tree and the drift is pre-existing at HEAD. | 3.1 | surface-pin refresh task (3.1-outcome.md §8: "owner: surface-pin task, NOT 3.1") | 📅 Forward | 3.1-outcome.md §3 + §8 carry-forward 2 + 3.1-PG §Suite 2; 5.1-outcome.md §1 step 4 + §2.1 + §5 item 3; 6.5-outcome.md §1; FINAL-outcome.md §3/§5/§9 | Not caused by, and not touched by, DEV1-015: zero schema/resolver/codegen files in the plan diff, so the drift is owned by other features' surfaces; the schema-surface baseline pins need a refresh owned by the surface-pin refresh task. Non-blocking for this plan. |
