...
- [ ] **0.2 Prerequisite verification & prose-phantom sweep (verify-then-claim — MANDATORY)**
  - Verify each against LIVE code; record one verified-fact row per item in the outcome; any missing required artifact → ❌ ledger entry blocking the dependent task:
    ...
    2. `backend/db/repo/audit/` — bundle shows ABSENT → expected CREATE.
    3. `frontend/views/admin/` and `app/(dashboard)/audit/page.tsx` — bundle shows ABSENT → expected CREATE.
    ...
    7. `shared/locale/adminUsers-namespace.parity.test.ts` existence (bundle shows ABSENT) → decides D13 verify-or-create.
    8. `frontend/providers/apollo/apolloCache.ts` registered policies vs the frozen inventory assertion in `apolloCache.test.ts:90-99` (reconcile the known `NotificationListPage` drift — VERIFY live set, never subtract).
    ...
    10. Confirm bundle anchors still hold: `AuditService.createAuditLog` (`backend/services/admin/audit.service.ts:34-42`), `assertActorAdmin` (`user-management.service.ts:121-149`), `toAuditActionType` (:56-75), `getActivity` (`admin-user.repository.ts:261-279`), `AuditActionTypePothosEnum` (`enum.pothos.ts:32-34`), `DateTime` scalar (`scalar.pothos.ts:1-4`), `withAuditDeleteTriggersSuspended` (`test/helpers/db-cleanup.ts:30-51`), `isPgliteProvider` (`test/helpers/skip-when-pglite.ts`), journey helpers barrel `@/test/workflows/helpers`, `navItems.ts:65` admin `/audit` item, seven `adminUsers.activity.action*` labels (`shared/locale/types/adminUsers/index.ts:206-221`), `formatApplicantDate` (`frontend/lib/i18n/format-date.ts:13-16`), `withPageAuth`/`roleDashboardPath`, `PermissionDeniedFallback`/`RetryableNotice` (real), `AppDataGrid` ABSENT (prose-only phantom — D10).
...
- [ ] **1.2 Conditional immutability trigger SQL migration (VERIFY-OR-CREATE per Task 0.2)**
  - **IF** Task 0.2 found an audit-immutability trigger under `backend/db/migration/**`: consume by reference; NO new file; record consumption in the outcome.
  - **IF ABSENT:** CREATE one new idempotent custom SQL migration under `backend/db/migration/**` per `docs/DATABASE_MIGRATIONS.md` (the sanctioned raw-SQL channel for triggers): `CREATE OR REPLACE FUNCTION` raising on `UPDATE`/`DELETE` of `audit_logs` + `DROP TRIGGER IF EXISTS` + `CREATE TRIGGER`. NO inline `--` comments inside any `sql` template (parameter-binding hazard). Apply to the migrate-capable dev DB via `bun db migrate`.
...
- [ ] **2.1 Shared admin gate + audit coercion extraction (`admin-gate.helpers.ts` — VERIFY-OR-CREATE, D2)**
  - **IF** Task 0.2 found `backend/services/admin/admin-gate.helpers.ts` existing (DEV3-022c landed first): import `assertActorAdmin` from it; ADD `toAuditActionType` extracted VERBATIM from `backend/services/admin/user-management.service.ts:56-75` (extend, never fork).
  - **ELSE:** CREATE `backend/services/admin/admin-gate.helpers.ts` carrying BOTH functions extracted VERBATIM: `assertActorAdmin(actorId, locale, outerTx?)` (from `user-management.service.ts:121-149`) and `toAuditActionType(raw)` (from :56-75).
...
- [ ] **2.2 [Write global audit-trail journey test — TEST-FIRST]**
  ...
  - Steps as sequential REAL service calls with REAL `actorUserId`s per specs §2.9 ordered step list:
    ...
    8. Teardown: tracked hard-delete — audit rows FIRST under `withAuditDeleteTriggersSuspended` (`test/helpers/db-cleanup.ts:30-51`), role children, then users — post-teardown re-probes assert ZERO residue.
  ...
  - If `test/workflows/` scaffolding were absent, this task would scaffold it (helpers + `test/workflows/AGENTS.md`) — Task 0.2 verified helpers exist via the `@/test/workflows/helpers` import anchor.
...
- [ ] **2.3 Implement `AuditTrailRepository` (`backend/db/repo/audit/`)**
  ...
  - Implementation contract: executor `(tx ?? db)` (precedent `admin-user.repository.ts:205-215`); ONE shared `buildWhere` with conjunctive parameterized `eq`/`gte`/`lt` conditions that drop out when absent; inner join `users` on `actor_id` projecting `actorName: users.fullName` (pattern from `getActivity`, `admin-user.repository.ts:266-278`); order `desc(createdAt), desc(id)`; `.limit().offset()`; `tx?: DBTransaction` LAST on every method.
...
- [ ] **2.5 Immutability proof triple (static scan + trigger tier + DDL idempotence)**
  - CREATE `backend/db/test/logic/audit/audit-immutability.test.ts` per `backend/db/test/AGENTS.md` conventions:
    ...
    - (b) **REQ-020 trigger tier (environment-branched):** probe `pg_trigger` (shape per `test/helpers/db-cleanup.ts:31-33`), gated by `isPgliteProvider()` (`test/helpers/skip-when-pglite.ts`): WITH triggers present → direct `tx.update(auditLogs)` and `tx.delete(auditLogs)` MUST throw inside `runInRollback`, asserted via `expectRepoError` try/catch (NEVER `rejects.toThrow`); WITHOUT triggers (push-provisioned) → assert the REQ-019 structural tier and RECORD the push-vs-migrate gap in the outcome.
...
- [ ] **3.1 Pothos objects/input + `adminAuditLogs` query registration**
  ...
  - CREATE `backend/graphql/query/admin/audit-trail.query.ts`: ONE `queryField("adminAuditLogs", …)` with `authScopes: { $all: { authenticated: true, role: [UserRole.Admin] } }` (precedent `admin-users.query.ts:38-42`); thin resolver: `ctx.user` belt + `UnauthorizedError((await ctx.t("errorsTranslations")).unauthorized)` (pattern `backend/graphql/mutation/notifications/notification.mutation.ts:32-34`), then EXPLICIT field-by-field copies into `AdminAuditTrailFiltersSubmitInput` (NO `{ ...input }` spread — BOPLA, REQ-032), passing `args.page ?? null`, `args.pageSize ?? null`, `ctx.locale`, `ctx.user.id`. NO try/catch, NO business logic.
...
- [ ] **4.1 Extend `adminUsers` namespace with the `auditTrail` block (NO new namespace — D13)**
  ...
  - REUSE the seven `adminUsers.activity.action*` labels (`shared/locale/types/adminUsers/index.ts:206-221`) for action rendering — mint NO near-duplicates (`shared/AGENTS.md` discipline).
  - **VERIFY-OR-CREATE** `shared/locale/adminUsers-namespace.parity.test.ts` per the Task-0.2 finding (bundle shows ABSENT): modeled on `shared/locale/handshakeCode-namespace.parity.test.ts` — key-set equality across locales, non-empty values, Arabic-script presence, registry/bundle wiring (`AdminUsers` handle + `adminUsersTranslations` on both bundles), and the NEW `auditTrail` block pinned under BOTH locales. Extend existing parity/growing-shape suites if present instead.
...
- [ ] **4.2 Frontend GraphQL documents + Apollo cache policy + contract tests**
  ...
  - UPDATE `frontend/providers/apollo/apolloCache.test.ts:90-99`: extend the frozen policy-inventory assertion in the SAME change set to the LIVE current set + `AdminAuditLogPage` (reconcile the known `NotificationListPage` drift — additive only).
...
- [ ] **4.3 Server-guarded route `app/(dashboard)/audit/page.tsx` (CREATE)**
  ...
  - Anonymous → `/login?redirect=/audit`; role mismatch → `roleDashboardPath(ctx.role)` (bare `/dashboard` FORBIDDEN). Zone locale via `getLocaleFromCookie()` (`shared/locale/server-cookies.ts:4-11`). Enum as VALUE import (`UserRole.Admin`).
...
- [ ] **4.5 Navigation retarget pinning (ZERO nav-model change — D12)**
  - `frontend/views/dashboard/navItems.ts` MUST NOT change — the existing admin item `{ route: "/audit", labelKey: "audit" }` (:65) is retargeted purely by Task 4.3/4.4 shipping the page.
  - UPDATE `frontend/views/dashboard/navItems.test.ts`: pin (a) admin nav CONTAINS `/audit` with `labelKey: "audit"`; (b) student/teacher/parent navs EXCLUDE `/audit`; keep the ownership-matrix test (:19-29) green (`audit` stays owned by `DashboardLabels`).
