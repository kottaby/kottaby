...
- **Non-goals (explicitly OUT of scope):**
  ...
  - **NO Drizzle schema changes.** `audit_logs` gains nothing. The ONLY conditional structural artifact is the DB-immutability trigger as a raw custom-SQL migration (verify-or-create, REQ-020) — the json
    sanctioned channel for triggers per `docs/DATABASE_MIGRATIONS.md`.
...
- **REQ-002 (Type-Safe i18n & Enum Value Imports Compliance):**
  ...
  - GraphQL resolvers MUST use `ctx.t("errorsTranslations")` (precedent: `backend/graphql/mutation/notifications/notification.mutation.ts:32`).
  - Services MUST use `getServerTranslations(locale)` from `@/shared/locale/server-graphql` (ONE argument — `shared/locale/server-graphql.ts:1-4`).
...
- **REQ-004 (Verification-First Reuse Guard — verify-then-claim):** WHEN domain work begins THEN the agent SHALL verify against the bundled/live code and REUSE (never fork):
  - the single writer `AuditService.createAuditLog` (`backend/services/admin/audit.service.ts:34-42`) — READ-ONLY reference;
  - `AuditLogWriteContract` (`backend/types/contracts/admin-audit.contract.types.ts:4-10`);
  - the admin gate `assertActorAdmin` — currently PRIVATE at `backend/services/admin/user-management.service.ts:121-149`. IF `backend/services/admin/admin-gate.helpers.ts` already exists (the DEV3-022c extraction direction) THEN import it; ELSE extract the function VERBATIM into that new module and re-import it in BOTH services (byte-equivalence guarded by DEV3-016's existing suites staying green);
  - the audit action coercion switch `toAuditActionType` (`backend/services/admin/user-management.service.ts:56-75`) — extract it VERBATIM into the new shared helper module and re-import it in both services (a second copy would trip `check:duplicates`);
  - the scoped-activity read precedent `AdminUserRepository.getActivity` (`backend/db/repo/admin/admin-user.repository.ts:261-279`) — projection + join + `desc(createdAt), desc(id)` pattern to mirror, not fork;
  - the registered Pothos enum `AuditActionTypePothosEnum` (`backend/graphql/pothos/shared/enum.pothos.ts:32-34`) — REUSE; re-registration is a runtime error;
  - the `DateTime` scalar (`backend/graphql/pothos/shared/scalar.pothos.ts:1-4` + builder Scalars slot) for all timestamp exposure;
  - the trigger-suspension teardown helper `withAuditDeleteTriggersSuspended` (`test/helpers/db-cleanup.ts:30-51`);
  - journey provisioning helpers (`provisionAdminActor`/`provisionStudentActor`/`provisionParentActor` via `@/test/workflows/helpers` — existence anchored by `backend/services/notifications/realtime/fanout-transport.test.ts:12`);
  - `withPageAuth` (`frontend/lib/auth/withPageAuth.ts:15-30`), `roleDashboardPath` (`frontend/lib/auth/roleDashboardRoute.ts:9-22`), `formatApplicantDate` (`frontend/lib/i18n/format-date.ts:13-16`), `PermissionDeniedFallback`, `RetryableNotice`;
  ...
  - the seven existing localized action labels (`adminUsers.activity.actionCreate|actionUpdate|actionDelete|actionReactivate|actionOverride|actionAdjust|actionSuspend` — `shared/locale/types/adminUsers/index.ts:206-221`).
...
- **REQ-030 (BFLA — Dual-Gate Admin Enforcement):** WHEN `adminAuditLogs` is invoked THEN `authScopes: { $all: { authenticated: true, role: [UserRole.Admin] } }` SHALL deny anonymous callers with `UNAUTHORIZED` and authenticated non-admins with `FORBIDDEN` BEFORE the resolver body executes (the load-bearing `$all` conjunction — `backend/graphql/query/admin/admin-users.query.ts:38-42` precedent), AND the service SHALL re-verify the actor: `actorId = 0` → `UnauthorizedError`; a resolvable non-admin actor → `ForbiddenError` (via the shared/extracted gate of REQ-004), executed BEFORE any read.
...
- **REQ-060 (Exact Operation Signature):** the schema SHALL gain EXACTLY ONE root field:
  ...
  carrying `authScopes: { $all: { authenticated: true, role: [UserRole.Admin] } }`. NO mutations. `backend/lib/gateway/public-operations.ts` stays the frozen six (`PUBLIC_OPERATION_NAMES` at `public-operations.ts:1-8`).
...
- **REQ-069 (Date Rendering Reuse):** `createdAt` SHALL render through the existing `formatApplicantDate` (`frontend/lib/i18n/format-date.ts:13-16`) — reused, never forked.
