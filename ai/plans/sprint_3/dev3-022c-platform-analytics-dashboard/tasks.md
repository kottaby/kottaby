...
- [ ] 0.2 [Prerequisite & reuse verification — verify-then-claim sweep]
  - Verify each of the following EXISTS in the bundled tree (cite `path:line` for each in the outcome file); if ANY is missing, log a ❌ deferred item and STOP affected downstream tasks — never inline-patch a foreign layer:
    - `AdminUserRepository.getStats` at `backend/db/repo/admin/admin-user.repository.ts:225-260` and its ACTIVE-window subquery semantics at `backend/db/repo/admin/admin-user.repository.ts:166-175`.
    - `assertActorAdmin` precedent at `backend/services/admin/user-management.service.ts:121-149` and the `withTransaction` import at `backend/services/admin/user-management.service.ts:13`.
    - The `$all` admin scope pattern at `backend/graphql/query/admin/admin-users.query.ts:38-43` (query tier) and `backend/graphql/mutation/admin/admin-users.mutation.ts:26-31`.
    - The `DateTime` scalar registration at `backend/graphql/pothos/shared/scalar.pothos.ts:1-3` and the builder `Scalars` slot at `backend/graphql/pothos/builder.ts:16-18`.
    - The journey harness `test/workflows/AGENTS.md` and helpers barrel `@/test/workflows/helpers` (existence anchor: import at `backend/services/notifications/realtime/fanout-transport.test.ts:12`); actor-context factories `provisionAdminActor` / `provisionStudentActor` / `provisionCertifiedTeacherActor` / `provisionParentActor`.
    - `withAuditDeleteTriggersSuspended` + `deleteUsersByIds` at `test/helpers/db-cleanup.ts:30-76`.
    - `withPageAuth` signature at `frontend/lib/auth/withPageAuth.ts:15-30`; `roleDashboardPath` at `frontend/lib/auth/roleDashboardRoute.ts:9-22`; `getLocaleFromCookie` at `shared/locale/server-cookies.ts:4-11`.
    - `NOTIFICATION_COUNT_POLL_INTERVAL_MS` at `frontend/components/ui/NotificationUnreadBadge.tsx:10` (polling cadence precedent).
    - `AdminUserStatsReturnType` at `backend/types/admin/admin-user.types.ts:36-47`.
    - Current `backend/db/test/entity-setup.ts:1-120` helper inventory + parameter convention — CONFIRM the ABSENCE of subscription/payment/session/report/evaluation/wallet/teacher-transaction factories (the verified gap that Task 1.3 fills).
    - `recharts` present in `package.json` dependencies.
    - Admin nav block at `frontend/views/dashboard/navItems.ts:58-67`; governance fields at `backend/db/schema/users/users.ts:16-23`; `session` columns at `backend/db/schema/classes/session.ts:5-30`; `student_payments` at `backend/db/schema/billing/student-payments.ts:6-25`; `subscriptions` at `backend/db/schema/billing/subscriptions.ts:5-27`; `teacher` at `backend/db/schema/teachers/teacher.ts:5-23`; `reports` rating check at `backend/db/schema/classes/reports.ts:20-23`; `evaluations` at `backend/db/schema/teachers/evaluations.ts:16-19,27`; `teacher_transaction` at `backend/db/schema/billing/teacher-transaction.ts:7-22`; pg enum mirrors at `backend/db/schema/enums.ts:5-37`.
    - CONFIRM the ABSENCE of `frontend/views/admin/**` and `app/(dashboard)/admin/**` (net-new CREATE — plan D12) and the absence of `frontend/views/admin/analytics/**`, `test/workflows/admin/**`, `test/ui/components/admin/**`.
...
- [ ] 1.3 [Fixture factories — UPDATE `backend/db/test/entity-setup.ts`]
  ...
  - Factories MUST honor real CHECK constraints: `studentRatingByTeacher` within 0–5 (`backend/db/schema/classes/reports.ts:20-23`), `score` within 0–100 (`backend/db/schema/teachers/evaluations.ts:27`), money as decimal strings, currency codes as strings, enum values via VALUE imports of enum members (never raw strings).
...
- [ ] 1.4 [i18n namespace `analytics` — full registration]
  - Files to create/modify (exact paths):
    ...
    - UPDATE `frontend/views/dashboard/navItems.ts` — admin block (anchored at lines 58-67) gains exactly ONE entry `{ route: "/admin/analytics", labelKey: "analytics", Icon: InsightsOutlined }` after the `/audit` entry. (Nav registration folded here so the dashboard label lands with its owner — REQ-065. The route target may 404 until Phase 4 — acceptable interim state recorded in the outcome.)
...
- [ ] 2.1 [Write Journey A — Cold platform honesty — TEST-FIRST]
  - Create `test/workflows/admin/platform-analytics.journey.test.ts` (the file ALL four journeys share; if the `test/workflows/admin/` directory does not exist this task creates it — the harness layer itself is verified present in 0.2, so no harness scaffolding is owed; if verification in 0.2 found the harness absent, this task additionally scaffolds helpers + `test/workflows/AGENTS.md` per Architectural Invariant 10).
...
- [ ] 2.5 [Implement Backend Repository — CREATE `PlatformAnalyticsRepository`]
  - Files to create:
    ...
    - Enum predicates via VALUE imports: `SessionStatus.*`, `SubscriptionStatus.*`, `PaymentStatus.Paid`, `TransactionType`/`TransactionStatus` members, `PaymentGateway.{OfflineCash, BankTransfer, Scholarship}` (`backend/db/schema/enums.ts:5-37` mirrors + `backend/enum/**`).
...
- [ ] 3.2 [Implement admin query + schema regeneration + baseline reconciliation]
  - Files to create/modify:
    ...
    - VERIFY `backend/lib/gateway/public-operations.ts:1-13` remains the frozen six (analytics is NOT public — REQ-030); `handshake-code-surface.test.ts` untouched and green.
...
- [ ] 3.3 [GraphQL wire matrix — CREATE `backend/graphql/test/platform-analytics.query.test.ts`]
  - In-process wire tests per the `backend/graphql/test/handshake-code-surface.test.ts:118-173` pattern; HTTP tier via the `setupTestServerLifecycle` precedent (`backend/graphql/test/notification-mutation.test.ts`) when the port window allows:
...
- [ ] 4.3 [Server-guarded page — CREATE `app/(dashboard)/admin/analytics/page.tsx`]
  - Files to create:
    - CREATE `app/(dashboard)/admin/analytics/page.tsx` (directory net-new — D12; no "extend" claims). Server Component wrapped by `withPageAuth({ roles: [UserRole.Admin], redirectTo: "/admin/analytics" })` (signature anchored at `frontend/lib/auth/withPageAuth.ts:15-30`); wrong-role fallback rides `roleDashboardPath(ctx.role)` (`frontend/lib/auth/roleDashboardRoute.ts:9-22` — bare `/dashboard` NEVER a target). `generateMetadata` resolves `getTranslations(locale).analyticsTranslations.metaTitle` / `.metaDescription` with locale from `getLocaleFromCookie()` (`shared/locale/server-cookies.ts:4-11`) — ONE-argument `getTranslations`, property access (REQ-003). Renders `<PlatformAnalyticsContainer />` only (server shell — zero data fetching on the server for this surface; the snapshot comes from the client query per REQ-062 discipline).
