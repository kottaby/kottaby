...
## ⚠️ Risks & watch items
- ~~The residency of the canonical `DateTime` scalar registration: verify registry vs each new Pothos file's imports during 4.x tasks.~~ **RESOLVED — see outcome/4.1 (registration section) + outcome/4.2 §2: the admin filter input rides the shared registry `DateTime` scalar by NAME; the canonical registration was reused and no second registration exists. Closed at 7.3 (R1 mid-point finding #7).**
- ~~No idempotency-claim decorator position verified for mutations at plan time — reuse the same mechanism the participant mutations use (chained inside `withTransaction` or outer claim wrapper; record finding in 4.3 outcome).~~ **RESOLVED — see outcome/4.3 §3: there is NO decorator and NO outer claim wrapper; the key threads resolver → service as a propagation-only argument (`x-idempotency-key` header → `ctx.idempotencyKey`) and the claim is inserted savepoint-bracketed INSIDE the service's `withTransaction` body. Closed at 7.3 (R1 mid-point finding #7).**
...
| D-08 | Idempotency claim uniqueness is global on `idempotency_key` alone (not `(key, user_id)`) — a second claimant of the same key string gets fail-closed `SESSION_NOT_FOUND` and a trivial key-collision oracle | post-impl pentest R2 | platform hardening stream (cross-cutting with booking's claim mechanism) | ⏭ Forward-owned | — | Inherited mechanism; booking depends on it; per-actor scoping is a shared-mechanism change out of this ticket |
| D-09 | Directory filter/order columns (status, session_type, created_at) lack supporting indexes; admin directory relies on seq-scan+sort (page ceiling now bounds offset) | post-impl pentest R2 | schema/index stream (this ticket is contractually no-schema-change per plan §2) | ⏭ Forward-owned | — | Admin-only surface behind rate limiting; add indexes when the session table's growth justifies a migration |
