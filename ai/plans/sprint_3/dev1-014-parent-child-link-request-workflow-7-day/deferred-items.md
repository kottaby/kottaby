...
| D9 | REQ-080 canonical doc must record (a) 6.2-F1 requireActor-before-withTransaction divergence from REQ-040 letter + (b) 6.4-F1 silent-expiry re-request UX tradeoff (D1 interplay) — owned by task 7.1 | 6.2 + 6.4 | Task 7.1 | 📅 Forward | — | Resolved-pointer: doc-content only, no code change. (a) the fresh actor re-check runs BEFORE `withTransaction` opens the unit on all three own-commit mutation paths (no atomicity/governance consequence — the re-check still precedes every write per REQ-031; see `outcome/6.2-review-backend-outcome.md` F1). (b) re-request after a silently lapsed (unmaterialized) pending row answers `PARENT_LINK_ALREADY_PENDING` while the outgoing list renders `Expired`, and the UI hides Cancel on both sides for such rows — the sanctioned cancel→EXPIRED→re-submit choreography is UI-unreachable until D1's cron sweep owns materialization; the CURRENT contract is pinned honestly by the 11th chaos cell ("re-request after silent expiry") added in 6-fixes-r2 |
