...
### 6.1 [ ] review-types wave — canonical types, enums, scalars
**Scope:** `backend/types/**`, `backend/enum/**`, `shared/constants/**`, Pothos object/enum registration
- [ ] Verify: no service-layer `.types.ts`; `StudentLinkTargetRowType` never leaks into GraphQL; `LinkStatus` registered ONCE ENUM-OBJECT form; timestamps all use `DateTime`; shared/constants has zero `@/backend/**` imports
- [ ] Write findings to `outcome/6.1-review-types-outcome.md` — any violation MUST be fixed before 6.5

### 6.2 [ ] review-backend wave — repos + services + resolvers
**Scope:** `backend/db/repo/**`, `backend/services/**`, `backend/graphql/**` for this ticket
- [ ] Verify each plan decision D1–D12 has a live artifact; REQ-011/012/013/014/015/016/017/018/019/020/021/023/024/030/031/032/033/034/035/040..044/050/053/054 all honored in code (not just comments)
- [ ] Verify guarded-UPDATE-only discipline (no SELECT-then-UPDATE in transition paths)
- [ ] Verify i18n: NO hardcoded strings in services/resolvers; ALL user-facing copy from `getServerTranslations`/`ctx.t`
- [ ] Verify log hygiene (REQ-035 forbidden fields absent; REQ-054 happy-path silence preserved)
...
- [ ] Verify `roleDashboardPath` only for wrong-role redirects (NO bare `/dashboard`)
- [ ] Verify RTL render paths from the 4.2.BS/4.3.BS screenshot finals
- [ ] Write findings to `outcome/6.3-review-frontend-outcome.md`

### 6.4 [ ] pentester wave — threat-model walkthrough against the shipped surface
**Scope:** REQ-030..037, REQ-053, REQ-054; the visibility matrix in plan §4.4
- [ ] Walk each threat class (BOLA, BFLA, BOPLA, oracle channels, LIKE surface, code-log leakage, governance-window honesty) and produce a pass/fail per row
- [ ] Re-probe the race conditions from §4.3 with fresh eyes (any missed interleave?)
- [ ] Verify smuggle/validation wire probes actually fail pre-resolver (not just at the service)
- [ ] Confirm: ZERO `audit_logs` rows produced by any of the five ops across all branches (silent-expiry, sibling-expiry included — REQ-024)
- [ ] Write findings to `outcome/6.4-pentester-outcome.md`

### 6.5 [ ] Deferred-items ledger closure gate
...
- [ ] 6.5.QL **Quality Loop:** outcome file `outcome/6.5-ledger-gate-outcome.md` written; gate value quoted verbatim

---

## Phase 7: Knowledge Propagation & Documentation

### 7.1 [ ] Canonical doc `docs/parents/parent-link-request.md` (house structure)
...
- [ ] 7.1.QL **Quality Loop:** `bun run scripts/health/sub-loop.ts docs/parents/parent-link-request.md --lifecycle duplicates` (exit 0)
- [ ] 7.1.SR **Semantic Review:** every RULE in the doc is backed by a test anchor (file:line) not just prose; forward-pointers named with owner + candidate ticket
- [ ] 7.1.IV **Instruction Verification:** `.agents/instructions/backend.instructions.md` (doc conventions)

### 7.2 [ ] Surgical doc crosslinks + AGENTS propagation + root references
**REQ:** REQ-081, REQ-082
...
- [ ] 7.2.QL **Quality Loop:** each updated file run through the sub-loop
- [ ] 7.2.SR **Semantic Review:** every edit is a MINIMAL single-line addition; no renumbering anywhere; old docs' rules preserved verbatim
- [ ] 7.2.IV **Instruction Verification:** `.agents/instructions/backend.instructions.md`

### 7.3 [ ] Outcome synthesis + completion gate
...
- [ ] 7.3.QL **Quality Loop:** final outcome file complete; checkbox sweep clean
- [ ] 7.3.SR **Semantic Review:** no requirement is claimed without a link to artifacts (test file / scan lock / wire assertion)
- [ ] 7.3.IV **Instruction Verification:** closing sweep across `.agents/instructions/{backend,frontend,tests}.instructions.md`
```
