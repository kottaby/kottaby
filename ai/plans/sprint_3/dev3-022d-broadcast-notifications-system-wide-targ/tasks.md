...
- [ ] 0.2 [Prerequisite verification — reuse substrate exists (REQ-002)]
  - Verify (grep/read, cite `path:line` in the outcome) each of: `NotificationEngine.emitForUsers` (`backend/services/notifications/notification-engine.service.ts:191`), `publishReceipts` (`:229`), `validateEmitBatchInput` (`backend/services/notifications/emit-validation.ts:64`), `buildEmitClaimKey`/`attemptEmitClaim`/`storeEmitReceiptQuietly` (`backend/services/notifications/emit-idempotency.ts:16,21,42`), `NotificationRepository.createManyReturning` (`backend/db/repo/notifications/notification.repository.ts:55`), `AuditService.createAuditLog` (`backend/services/admin/audit.service.ts:34`), `assertActorAdmin` (`backend/services/admin/user-management.service.ts:121`), `PlanRepository.existsById` (`backend/db/repo/billing/plan.repository.ts:55`), active-subscription predicate (`backend/db/repo/admin/admin-user.repository.ts:166-174`), `withTransaction`, `SpiedFanoutTransport` in test helpers, `projectMutationFieldErrors` (`frontend/lib/mutationFieldErrors.ts`), `withPageAuth` (`frontend/lib/auth/withPageAuth.ts`), `adminPlansQueryDocument`.
  - IF any artifact is missing → record `❌` in `deferred-items.md` and BLOCK; do NOT re-implement the engine.
  - Confirm `test/workflows/` layer existence (if absent, task 2.0 scaffolds it per Architectural Invariant 10).
...
- [ ] 1.1 [Create `BroadcastAudienceType` TS-only enum + fail-closed guard]
  - CREATE `backend/enum/notifications/broadcast-audience-type.enum.ts` — `enum BroadcastAudienceType { All="all", Role="role", Country="country", Plan="plan" }` + `isBroadcastAudienceType(value: unknown)` guard (mirror `backend/enum/notifications/notification-type.enum.ts:10`). TS-only; NO `pgEnum` in `backend/db/schema/enums.ts`.
...
- [ ] 1.2 [Create canonical broadcast types + widen audit contract]
  ...
  - UPDATE `backend/types/contracts/admin-audit.contract.types.ts` — widen `AuditLogWriteContract.entityId` to `AuditLogSelectType["entityId"]` (schema-derived `number | null`; `audit_logs.entity_id` already nullable at `backend/db/schema/audit/audit-logs.ts:13`). Additive widening ONLY (DB-5); no other contract shape change.
...
- [ ] 1.3 [Register four new `ErrorsLabels` keys (broadcast domain, flat)]
  - UPDATE `shared/locale/types/errors/index.ts` — add flat, domain-prefixed keys: `broadcastTitleInvalid`, `broadcastAudienceInvalid`, `broadcastAudienceEmpty`, `broadcastAudienceTooLarge` (NO nested groupings — `ErrorsLabels` is flat).
...
- [ ] 2.0 [Journey: `test/workflows/notifications/admin-broadcast.journey.test.ts` — TEST-FIRST]
  ...
  - Scaffold the layer if absent: `test/workflows/AGENTS.md` + `test/workflows/helpers/` (cast provisioning: `provisionAdminActor`, `provisionCertifiedTeacherActor`, `provisionStudentActor`, `provisionParentActor`, governed user with `is_deleted=true`; REAL permission-group/role rows — permission resolution NEVER monkey-patched) per Architectural Invariant 10.
...
- [ ] 2.2 [Extract shared `assertActorAdmin` + refactor `AdminUserManagementService`]
  - CREATE `backend/services/admin/assert-actor-admin.ts` — move the logic from `backend/services/admin/user-management.service.ts:121-149` VERBATIM (anonymous `actorId = 0` → `UnauthorizedError`; missing/non-admin row → `ForbiddenError`; identical logging shape; pre-transaction; zero writes/audit — JR-C-1).
...
- [ ] 3.2 [Create `adminBroadcastNotification` mutation + codegen + baseline freeze updates]
  - CREATE `backend/graphql/mutation/notifications/admin-broadcast.mutation.ts` — side-effect module; field `adminBroadcastNotification(input: AdminBroadcastNotificationInput!): Int!` with `authScopes: { $all: { authenticated: true, role: [UserRole.Admin] } }` (the `$all` conjunction is load-bearing — precedent `backend/graphql/mutation/admin/admin-users.mutation.ts:26-30`); resolver: anonymous guard via `ctx.t("errorsTranslations")` → `UnauthorizedError(tErrors.unauthorized)`; delegate EXCLUSIVELY to `AdminBroadcastService.broadcast({ title, body, audience }, ctx.user.id, ctx.locale, ctx.idempotencyKey ?? undefined)`; NO try/catch (boundary-only masking); NO direct engine calls.
