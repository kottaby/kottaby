...
- [ ] 2.1 Trigger amendment migration (D-2) + duplicate-dir verification
  - Verify the two `custom_4-student-payments-status-transition` dirs (`backend/drizzle/20260907182426_…` / `20260908103411_…`) hold identical payloads; document findings (resolves ledger D1)
  - Author `backend/db/migration/5-teacher-transaction-settlement.sql` + `5-teacher-transaction-settlement-sqlite.sql`: `CREATE OR REPLACE FUNCTION prevent_teacher_transaction_update()` permitting ONLY `pending AND type='withdrawal' → completed|failed` with ALL other columns frozen (`IS NOT DISTINCT FROM`)
  ...
  - Update the schema docblocks that describe the trigger (`backend/db/schema/billing/teacher-transaction.ts:17-22`) to document the settlement exception
...
- [ ] 2.3 Enum + repository primitives + 100% repo tests
  ...
  - Tests: extend `backend/db/test/repo/billing/` suites — filter matrix, wrong-state misses, guarded-debit miss, freeze violations (via crafted UPDATEs), pagination bounds; run via `bun run test/scripts/run-test.ts`
...
- [ ] 2.4 Journey test authored TEST-FIRST (expected red until 2.5 lands)
  ...
  - Per `test/workflows/AGENTS.md`: real services + real DB, committed fixtures via actor-context helpers + tracked `afterAll` cleanup (audit rows via `withAuditDeleteTriggersSuspended`), NO `runInRollback`, no `expect.rejects`, per-run `jrn_adminfin_<8hex>` prefix
...
- [ ] 2.5 `AdminFinancialAuditingService`
  - New `backend/services/billing/admin-financial-auditing.service.ts` (+ `.helpers.ts`): the six methods from plan.md Component 2; `assertActorAdmin` first inside `withTransaction`; pagination via `resolvePageBounds`; audit contract builders mapping D-4 vocabulary; writes via `AuditService.createAuditLog(contract, tx)` only
...
- [ ] 4.3 Admin finances console UI + nav
  ...
  - UI tests `test/ui/components/admin-finances/` (Happy DOM, both locales, loading/403/error/empty/populated, dialog mutation-variable assertions)
