...
> **Blocking dependencies:** DEV1-001 (`plans` table + CHECK constraints + `bun validate:dbml` pipeline), DEV1-002 (registration + `createAdminUser` service path), DEV2-001 (JWT context factory: `ctx.user` / `ctx.role`), DEV2-002 (`role` authScope with OR semantics, fail-closed evaluation).
> **Critical reconciliation note:** The `plans` table delivered by DEV1-001 (`backend/db/schema/billing/plans.ts`, mirrored in `db/schema.dbml`) contains **no activation/status column**. The ticket's acceptance criteria require "activate / deactivate" semantics. This ticket therefore carries an explicitly authorized **schema delta**: `plans.is_active boolean NOT NULL DEFAULT true` and `plans.deactivated_at timestamp NULL`, applied via `bun run db push` with `db/schema.dbml` updated in the same unit of work (`bun validate:dbml` must stay green) — following the DEV1-004 precedent for per-ticket schema deltas (`balance_trial`/`trial_granted_at`). The addendum is recorded in `docs/specs/open-decisions-and-gaps.md` (REQ-081). No other structural drift is permitted.
...
- **REQ-010 (Schema Delta — Plan Lifecycle Columns)**: WHEN this ticket is implemented THEN the `plans` table SHALL gain exactly two columns: `is_active boolean NOT NULL DEFAULT true` and `deactivated_at timestamp NULL` (no default). The change SHALL be applied exclusively via `bun run db push` (repo policy: `db reset`/`db cleanGenerate` permanently disabled), `db/schema.dbml` SHALL be updated in the same unit of work with decision-referenced notes, and `bun validate:dbml` SHALL pass. No new tables, enums, or indexes SHALL be created (the catalog is a small lookup set; the partial-scan cost of `WHERE is_active` is trivially acceptable — recorded as a decision note, not an omission).
...
- **REQ-042 (Schema Application Discipline)**: WHEN the schema delta lands THEN Drizzle schema (`backend/db/schema/billing/plans.ts`), `db/schema.dbml`, and canonical types SHALL be updated in the same commit set to prevent drift; `bun validate:dbml` and `bun tsgo` SHALL both pass, and the new columns SHALL flow into `PlanSelectType`/`PlanInsertType` automatically via `$inferSelect`/`$inferInsert`.
...
- **REQ-077 (Quality Gates)**: WHEN tasks complete THEN `bun run scripts/health/sub-loop.ts <file> --lifecycle duplicates` SHALL exit 0 for every created/modified file; schema docs (`bun validate:dbml`) SHALL stay green; and the REQ-020 no-delete-surface grep assertion SHALL be part of the verification.
...
| REQ-010 / REQ-042 | Schema-delta addendum; DBMigrations policy | `backend/db/schema/billing/plans.ts` delta + DBML same commit; types flow via `$infer*` | — | — | `bun run db push` log; `bun validate:dbml` green; column-presence test |
