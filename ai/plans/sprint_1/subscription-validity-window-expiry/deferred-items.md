...
| D2 | Scheduler deployment trigger for `/api/cron/expire-subscriptions` | specs REQ-020 | Final deployment/ops task | ✅ Done | outcome/9.1-outcome.md · `docs/billing/subscription-validity-window-expiry.md` | CLOSED at 9.1 — the external-trigger ops handoff is now canonically documented: the route is live-but-unused until ops wires an external cron caller (host cron / scheduler) hitting `GET /api/cron/expire-subscriptions` with `Authorization: Bearer ${CRON_SECRET}`, gated on deployment env `CRON_EXECUTION_MODE=external` + `CRON_EXTERNAL_ENABLED=true`; no in-process scheduler exists by design; kill switch = flip `CRON_EXTERNAL_ENABLED` off (bare 404); manual ops invocation uses the same curl-shaped call; duplicate deliveries are replay-safe (`{ expired: 0, lanesZeroed: 0 }`) |
