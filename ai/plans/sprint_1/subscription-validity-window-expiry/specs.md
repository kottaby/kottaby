...
- **REQ-022 (Guarded Batch Transition)**: WHEN the sweep updates rows THEN the repo method SHALL be a single guarded `UPDATE … SET status = 'expired' WHERE status = 'active' AND end_date IS NOT NULL AND end_date <= now()` (SQL-side comparison, UTC timestamps) whose zero-row outcome is the replay/no-op branch — guarding doctrine per house style (`activatePendingOnce` precedent, `subscription.repository.ts:131-149`); re-running the sweep SHALL change nothing; a second concurrent sweep SHALL never produce a different terminal state (idempotent convergence). plan.md SHOULD address sweep efficiency (no index on `status`/`end_date` exists today — `subscription.repository.ts:38-44` indexes only `user_id`/`plan_id`/partial `payment_reference`).
- **REQ-023 (AC2 Balance Zeroing — semantic DECISION REQUIRED in plan.md)**: WHEN a subscription expires THEN that period's remaining session balance SHALL be zeroed (INV-B3: unused sessions expire at window end, no carryover — `docs/specs/state-machine-invariants.md:147`). BUT the shipped balance model is flat per-student lanes with NO per-subscription attribution (`students.ts:24-45`), so literal per-period zeroing is unimplementable today. plan.md MUST decide and justify ONE implementable semantic before implementation begins, choosing between (or a documented variant of): **(a) conservative lane-conditional zeroing** — zero the expiring plan's credited lane ONLY when no other in-window active subscription covers that student-lane pair; **(b) per-subscription allocation ledger** — new schema tracking credit provenance enabling exact per-period zeroing. The decision, its trade-offs vs INV-B3's wording, and rejection of the alternatives SHALL be recorded in plan.md's Key Design Decisions table and ratified in the outcome ledger; deferred-items D1 tracks this. The specs deliberately do NOT smuggle the choice in.
...
- **REQ-061 (Booking-Dialog Error Arm — optional, fallback-first)**: WHEN `SUBSCRIPTION_EXPIRED` reaches the booking dialog THEN the EXISTING VALIDATION fallback path (`frontend/providers/apollo/error-link.map.ts` code→surface map → localized snackbar via `sessionDialogErrorArms.ts` vocabulary) SHALL render the localized copy with ZERO frontend changes (verified: custom domain codes fall through to the VALIDATION fallback; no `INSUFFICIENT_BALANCE` consumer exists in `frontend/` either). A dedicated error arm in `frontend/views/student/sessions/sessionDialogErrorArms.ts` is OPTIONAL and deferred (deferred-items D3) unless plan.md finds a cheap slot.
...
**No new page routes, no new GraphQL operations.** The booking denial surfaces inside the existing student sessions page (`app/(dashboard)/student/sessions/page.tsx` → `StudentSessionsContainer`) via the existing error-link/snackbar machinery.
...
| Teacher | ❌ | none | none |
| Parent | ❌ | none | none |
| Student | ❌ | `SUBSCRIPTION_EXPIRED` denial on expired subscription-lane funding | localized snackbar (existing VALIDATION fallback) |
