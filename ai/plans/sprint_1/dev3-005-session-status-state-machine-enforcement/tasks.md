...
> **Ledger:** `ai/plans/sprint_1/dev3-005-session-status-state-machine-enforcement/deferred-items.md` · **Outcomes:** `ai/plans/sprint_1/dev3-005-session-status-state-machine-enforcement/outcome/`
...
- [ ] 0.1 Record baseline + init ledger
...
- [ ] 0.2 Ground-truth verification sweep (READ-ONLY)
...
- [ ] 1.1 Create `backend/services/classes/session-lifecycle.enforcement.ts`
  ...
  - [ ] 1.1.QL **Quality Loop**: `bun run scripts/health/sub-loop.ts backend/services/classes/session-lifecycle.enforcement.ts --lifecycle duplicates` (+ any new repo file) — exit 0.
  - [ ] 1.1.TE **Tests** (`backend/services/classes/session-lifecycle.enforcement.test.ts`, `runInRollback`): Tier 1 — every matrix edge allow/deny truth table; gate pass paths. Tier 2 — gates on `disputed`/`cancelled`/unknown-id sessions; report-missing vs report-present. Tier 3 — concurrent `assertSessionCompletedForReport` calls during a racing completion (`Promise.allSettled`). Tier 4 — gates after tx rolled back (no phantom pass).
  - [ ] 1.1.SEC **Security**: ids are server-derived (no client shape accepted); denial vocabulary reuses existing codes; no existence oracle widening (gates are internal-only, participation checked by callers).
  - [ ] 1.1.SR **Semantic Review**: zero dead exports; no inline matrix duplicates elsewhere; enums value-imported.
  - [ ] 1.1.IV **Instruction Verification**: validate against `.agents/instructions/backend.instructions.md` + auto-discovered AGENTS.md printed by sub-loop.
- [ ] 1.2 i18n keys (en/ar parity)
  ...
  - [ ] 1.2.QL / TE (parity test via run-test) / SEC (no leakage in messages) / SR / IV — same pattern as 1.1, scoped to the three edited files.

## Phase 2 — INV-S6 In-Session Lock Wiring

- [ ] 2.1 `TeacherRepository.setOnline` (guarded write)
  ...
  - [ ] 2.1.QL **Quality Loop** on the repo file — exit 0.
  - [ ] 2.1.TE **Repo tests** (`backend/db/test/repo/teachers/teacher.repository.test.ts`, EXTEND): set false on online teacher ⇒ flip; set true ⇒ flip; unknown id ⇒ null; redundant same-value write ⇒ row returned, no-op value change; executed under `runInRollback` with `tx` propagation proven by deadlock-free completion.
  - [ ] 2.1.SEC / SR / IV per protocol.
- [ ] 2.2 Compose lock into `startSession`
  ...
  - [ ] 2.2.QL / TE (service test: start ⇒ online false; roll-back fault injected ⇒ no lock persisted) / SEC / SR / IV.
- [ ] 2.3 Compose release into `completeSession` / `cancelSession` / `resolveSessionDispute`
  ...
  - _Requirements: REQ-2, REQ-5_
  - [ ] 2.3.QL / TE (complete-from-started ⇒ unlock; cancel-from-scheduled ⇒ no lock touch; resolve-from-started-dispute ⇒ unlock; resolve-from-scheduled-dispute ⇒ none; forced mid-tx fault ⇒ full rollback incl. teacher row) / SEC / SR / IV.

## Phase 3 — Journey Verification (Dispute + Lock + Denials)

- [ ] 3.1 New journey `test/workflows/sessions/session-state-machine.journey.test.ts`
  ...
  - [ ] 3.1.QL / TE (Tier 1–4 per specs; races via `Promise.allSettled` on duplicate resolve) / SEC (non-admin resolve denied `FORBIDDEN`; participant-only ops deny foreign actors) / SR / IV.
- [ ] 3.2 Regression: existing suites remain green
  ...
  - [ ] 3.2.QL / IV per protocol.

## Phase 4 — Docs, Ledger Close, Knowledge Propagation

- [ ] 4.1 Documentation amendments
  ...
  - [ ] 4.1.QL (sub-loop on edited md files — duplicates lifecycle) / SR / IV.
- [ ] 4.2 Final gate & ledger close
  - Re-run full-file quality loop for every file touched this plan; confirm baseline counts unchanged; `grep -c "❌\|⚠️" ai/plans/sprint_1/dev3-005-session-status-state-machine-enforcement/deferred-items.md` audit (any residual must reference an owning ticket).
