...
|----|---------------|-------------|-------------|--------|-------------|-------|
| D1 | Payment-failed aftermath for verification purchases: on a `failed` webhook the applicant stays `in_evaluation` with a `pending` subscription; re-purchase from `in_evaluation` is allowed (flip no-op) so no trap — formal UX for "payment failed, try again" is out of ticket scope | Design D1 | post-Sprint-1 UX/payment plan | ✅ Done | Task 13 | Accepted posture; documented in specs REQ-5.6; accepted posture documented in docs/teachers/verification-plan-purchase.md status table + specs REQ-5.6 |  |
| D2 | Paymob port amendments A1–A3 (`PaymentCheckoutInput.specialReference/billing`, `parseWebhookEvent(input)` signature change) may land while DEV2-005 is in flight → rebase sync point | Design (context) | paymob-gateway-integration merge point | ✅ Done | Task 13 | Code only against `PaymentGatewayPort`; adapter seam unchanged today; hand-off note recorded in the paymob merge-point context (outcome files); code only against PaymentGatewayPort |  |
| D3 | Student-role exposure: the seeded verification plan is active in the student catalog (students can purchase it via `purchaseSubscription`); restricting catalog by audience is NOT in this ticket | Design (context) | future catalog-visibility ticket | ✅ Done | Task 13 | Pre-existing DEV1-006 posture; documented in specs §4 Out of Scope; student-catalog exposure documented in the canonical doc + specs §4 out-of-scope; future visibility ticket owns the restriction |  |
| D4 | Activation for verification subscriptions intentionally skips lane crediting; DEV2-006 (5-session loop) must enforce the 5 distinct-evaluator bookings against the ACTIVE subscription | Design D4 | DEV2-006 | ✅ Done | Task 13 | Cross-ticket hand-off note; hand-off note recorded in docs/teachers/verification-plan-purchase.md (booking flow enforces the 5-session grant) + outcome 6 |  |
...
| D11 | Purchase-flow governance depth: the verification service asserts `assertActorGovernanceClean` pre-gateway only; the sibling student flow also re-asserts inside the tx (caller governed during the checkout round-trip rolls back there). Optionally add an active-purchase single-flight guard for repeat purchases from `in_evaluation` | Post-implementation review R1 (finding 1/4) | future defense-in-depth ticket | 🔄 In Progress | — | Plan §4.3-conformant as shipped; fail-open window is millisecond-scale on a governed-actor transition |
| D12 | Dialog confirm latch: after the mutation settles and before `onClose()`, the confirm affordance re-enables for one refetch window; a double-click could mint a second pending pair (server posture accepted per REQ-9.7 — guarded flip no-ops, settlement grants no extra credit) | Post-implementation review R1 (finding 2) | future UX-hardening ticket | 🔄 In Progress | — | Cheap `settling` latch suggested |
