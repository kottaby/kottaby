...
- Gates: `bun tsgo` / `bun biome:check` / `bun run scripts/lint-service.ts` delta = +0 vs REQ-001 baseline (REQ-079); `bun validate:dbml` green with empty schema diff (REQ-047); `bun run generate:gqlSchema && bun codegen` artifacts committed (REQ-064).
