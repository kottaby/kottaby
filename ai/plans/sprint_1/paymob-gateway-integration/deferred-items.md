...
| D-515 | The new cron route answers TWO bare-404 fail-closed gates (disabled cron mode; unconfigured paymob provider/API key) under classification `envelope`, and the error-handling contract's exemptions inventory (`docs/graphql/error-handling-contract.md`:94-102) carries NO row for cron-route 404 gates — the sweep-sessions precedent documents that exemption ON THE ROUTE, with the contract reserving inventory rows for the provider-ack webhook surface whose classification IS the exemption vehicle; task 5.2 followed the precedent exactly (no doc change) | 5.2 | 9.2 — adjudicated (mid-point review D-515): doc note only, no inventory row change — a one-sentence discoverability note naming both cron routes' bare-404 gates was added to `docs/graphql/error-handling-contract.md` §3 | ✅ Done | `outcome/9.2-outcome.md` (the discoverability note landed in `docs/graphql/error-handling-contract.md`; no inventory row added — the existing wording does not warrant one) | Resolved: the gate classification stays pinned by the route docblocks + the registry row; the contract now points readers at it |
| D-711 | The purchase-funnel documents (task 7.1) surface two NEW embedded value-object GraphQL types with no `id` — `PurchaseSubscriptionPayload` and `PaymentCheckout` (backend design: the normalizable entities are the nested `StudentSubscription`/`StudentPayment` rows); `frontend/graphql/AGENTS.md`'s embedded-type normalization policy requires `keyFields: false` typePolicies entries in `frontend/providers/apollo/apolloCache.ts` for such types, but that file is OUTSIDE 7.1's scoped file contract (documents + barrel + codegen outputs + snapshot tests only) | 7.1 | 7.2 (the first mutation consumer — add both typePolicies entries when wiring `usePurchaseSubscription`) / 5.6 (mid-point review) / 9.1 (final gate) | ✅ Done | `outcome/7.2-outcome.md` (both `keyFields: false` entries landed in `frontend/providers/apollo/apolloCache.ts` with the first real mutation consumer; the `apolloCache.test.ts` FROZEN-surface pin extended 18 → 20 entries and its two new `keyFields` exposure tests pass — 14/0) | Resolved: Apollo no longer warns on repeated purchase-mutation writes; the policy surface pin keeps the 20-entry list frozen |
| D-611 | `shared/AGENTS.md`'s "Namespace Registration" steps 3–5 still prescribe the REMOVED legacy loader's wiring (`MessageSchema` in a `types/message.ts` map, `namespacePaths` in `serverLegacy.ts`, `LocaleProvider` translations in `app/[locale]/layout.tsx` — none of those files/concepts exist in the live compile-time system); task 6.1 followed the LIVE convention instead (wallet/plans templates: `types/<ns>` interface + `en`/`ar` leaves + `namespaces/<ns>/<ns>.namespace.ts` handle + barrel/registry + `Translations`/messages registration + parity test) | 6.1 | 9.2 — adjudicated (mid-point review D-611: escalated into knowledge propagation): the Translation System section was rewritten to the live handle architecture at 9.2 (steps 1–6 + File Structure + the stale string-call examples fixed; `MessageSchema`/`namespacePaths`/`serverLegacy.ts`/`app/[locale]/layout.tsx` prose removed) | ✅ Done | `outcome/9.2-outcome.md` (the rewrite landed in `shared/AGENTS.md`; the live exemplars — wallet/plans/checkout namespaces — verified against the code before writing) | Resolved: every landed namespace follows the live convention and the prose now matches it |
...
**Consumed status (recorded at 9.2):** all six amendments (A1–A6) were landed and consumed during tasks 2.1–4.2 of this plan — the canonical gateway doc is `docs/billing/paymob-gateway.md`, and the amended contract surfaces are `backend/types/billing/` (A1/A2), `parseWebhookEvent`'s query input (A3), `findStalePendingByGateway` (A4), `getPaymobConfig()` (A5), and the activation service's failure branch + `docs/billing/subscription-purchase.md` §7 (A6).
