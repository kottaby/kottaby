...
**Feature:** `session-report-homework-infrastructure`  
**Plan Directory:** `ai/plans/sprint_1/session-report-homework-infrastructure`  
**Created:** `2026-09-05`
...
| D2 | Parent report read surface (parent `sessionReport`/`sessionHomework` visibility via `students.parent_id`) | specs §Traceability | the parent-portal ticket (parent portal) | 📅 Forward (DEFERRED) | — | Parent reads stay `null` by design in this ticket. |
| D3 | Teacher report submission/browsing UX (submit form, report views) | specs §Traceability | the submit-UX ticket | 📅 Forward (DEFERRED) | — | This ticket ships typed GraphQL documents only. |
| D4 | Aggregating `teacher.average_rating` from `reports.student_rating_by_teacher` | specs §Traceability | the rating-aggregation ticket | 📅 Forward (DEFERRED) | — | Owner: the rating-aggregation ticket rating aggregation. |
