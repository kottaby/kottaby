...
- [ ] 3.1 Frontend: student confirm affordance on completed rows (SessionRow action matrix + in-flight slot), teacher confirm state display, i18n ar/en + parity. *(owner: next round)*
- [ ] 4.1 Full gates + live loop: tsgo/biome/eslint/oxlint, battery delta 0, agent-browser confirm flow + sweep verification. *(orchestrator)*
- [ ] 5.1 Close-out: final outcome, deferred sweep, push, worklog. *(orchestrator)*
