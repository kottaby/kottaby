...
| D2 | Recipient-locale copy storage (localized notification copy) | 0.1 (DEV3-010 baseline) | emitter wiring tickets (D1 family) + frontend locale-sync wave (2b) | 🔄 Deferred | R2-users-locale-a backend suites (user-repo 11/0, auth-service 6/0, schema-surface 33/0, sdl-static 18/0, enum parity 8/0) | column + mutation + repo helpers SHIPPED in R2-users-locale-a (D2 backend vertical: `users.locale` `app_locale` pgEnum column, `AppLocale` enum + `User.locale`, `updateMyLocale` mutation, `UserRepository.updateLocale`/`findLocalesByIds`); emitter consumption + UI locale sync REMAIN with the owning tickets; never patched inline by an unrelated emitter ticket |
