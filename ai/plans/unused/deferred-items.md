...
| D5 | VercelObservability.tsx is a knip-flagged unused file; deleting it orphans `@vercel/analytics` + `@vercel/speed-insights` (currently kept + ignoreDependencies) | T1.2 | Phase 2 (unused files) | 🔄 In Progress | — | Decide the file's fate, then drop the two deps + ignore entries together |
