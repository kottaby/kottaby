...
> **Plan artifacts**: `ai/plans/ci-cd-pipeline/`
...
1. **Pre-Execution Outcome Knowledge Read (MANDATORY)**: Before touching any file for task `X.Y`, the executing agent reads **all** existing files in `ai/plans/ci-cd-pipeline/outcome/` in index order, plus `ai/plans/ci-cd-pipeline/deferred-items.md`. Cross-task carry-forward notes (e.g., discovered packaging gaps in validators) are binding.
...
5. **Outcome Documentation**: On task completion write `ai/plans/ci-cd-pipeline/outcome/<task-id>-outcome.md` containing: what was built, files touched, verification evidence (command outputs/screenshots), deviations from plan (if any), carry-forward notes for downstream tasks.
...
- [x] 0.1 Record pre-implementation baseline and initialize the deferred-items ledger
  - **Files to create**:
    - `ai/plans/ci-cd-pipeline/deferred-items.md` (instantiated from `.agents/spec-process-guide/templates/deferred-items-template.md`)
    - `ai/plans/ci-cd-pipeline/outcome/phase0-baseline-outcome.md`
...
- [x] 1.1 Record the zero-schema-change acknowledgement
  - **Files to create**: `ai/plans/ci-cd-pipeline/outcome/1.1-schema-na-outcome.md`
...
- [x] 7.3 Final synthesis and completion gate
  - **Files to create**: `ai/plans/ci-cd-pipeline/outcome/plan-completion-synthesis.md`
