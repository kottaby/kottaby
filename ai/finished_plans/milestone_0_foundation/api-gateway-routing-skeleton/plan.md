# Technical Architecture & Implementation Design: API Gateway & Routing Skeleton

> **Plan of record:** `ai/plans/api-gateway-routing-skeleton/`
> **Specs:** `specs.md` REQ-001..REQ-083
> **Canonical refs:** `docs/graphql/error-response-contract.md` (the Shared Error Handling & Response Contracts ticket), `docs/auth/jwt-authentication-service.md` (the JWT Authentication Service ticket), `docs/graphql/domain-error-extensions-code.md`, `docs/IDEMPOTENCY.md`, `docs/auth/user-registration.md`, `docs/auth/qiraah-selection-and-c5.md`, `docs/DATABASE_MIGRATIONS.md`, `docs/graphql/dataloader-batching.md`
...
│ STEP 2 requestId = resolveRequestId(headers) (honor X-Request-Id or UUID) │
│  STEP 3  idempotencyKey = headers["X-Idempotency-Key"]  (propagation only — docs/IDEMPOTENCY)   │
│ STEP 4 ctx = gqlContextFactory(request) (token/session verify, │
...
│ notImpersonating (403) (the Role-Based Authorization Middleware ticket — runs INSIDE engine, │
│                                                          per-field, fail-closed)                │
│  STEP 6  resolver → Service → Repository → PostgreSQL   (resolvers: argument mapping only)      │
│ STEP 7 a) finalizeGraphqlErrors(result, {locale, requestId}) (the Shared Error Handling & Response Contracts ticket mask/passthrough) │
...
│ 200 { data: {...}, requestId } (the Shared Error Handling & Response Contracts ticket envelope helper) │
└─────────────────────────────────────────────────────────────────────────────────────────────────┘
...
sequenceDiagram
    ...
    participant CF as gqlContextFactory (the JWT Authentication Service ticket)
    participant SC as scopeAuth/authScopes (the Role-Based Authorization Middleware ticket)
    participant SV as Service → Repository → DB
    participant FB as finalizeGraphqlErrors (the Shared Error Handling & Response Contracts ticket)
...
| D5 | **Transport guards are pure result-returning helpers** — `TransportResult = { ok: true, body: unknown } \| { ok: false, kind: TransportErrorKind }`; the route handler maps the kind to the Shared Error Handling & Response Contracts ticket envelope. | (a) throw DomainError (b) discriminated-union result (c) Next.js default 405 (unexported methods) | (a) Transport failures are not domain failures; conflating them muddies the taxonomy (REQ-016) and would force a fake resolver context before the engine exists. (b) No throw-as-control-flow; trivially unit-testable to 100% branch coverage, no try/catch needed. (c) Loses the envelope, the `requestId`, and the `Allow: POST` header contract (REQ-015). | (b). Guarantees that transport failures carry `requestId` and a localized generic message while never touching the GraphQL engine (REQ-010 step 1, REQ-014/015). |
...
Any discovered schema gap is escalated via `deferred-items.md` ❌ and owned by the Database Schema Migration ticket — never patched inline.

### 2.2 Canonical Types (NEW — `backend/types/gateway/`)

All types follow `backend/types/AGENTS.md`: no runtime logic in `.types.ts`, barrel `backend/types/gateway/index.ts` uses `./` relative `export *`, and root `backend/types/index.ts` gains `export * from "./gateway";`. These are transport-contract types, not DB-row types, so `{Entity}SelectType`/`InsertType` naming does not apply (same precedent as the Shared Error Handling & Response Contracts ticket's `backend/types/errors/`).
...
export interface GatewayRequestMetadata {
  readonly requestId: string; // the Shared Error Handling & Response Contracts ticket resolveRequestId contract
...
**None.** Health payload fields are scalars; `TransportErrorKind` is a TS string-union (transport metadata, never persisted, never GraphQL-exposed) — the Shared Error Handling & Response Contracts ticket D3 precedent applies. `backend/enum/**` and `backend/db/schema/enums.ts` untouched; no Pothos enum registration.
...
| Domain failure inside resolver | **200** (Apollo convention) | `errors[].extensions.code` per the Shared Error Handling & Response Contracts ticket taxonomy | masked iff non-DomainError |
...
export const PUBLIC_OPERATION_NAMES = [
  "login", "refreshToken", "logout", // the JWT Authentication Service ticket auth surface (logout clears cookies even when token expired)
  "registerUser", // the User Registration ticket (RegisterPublicRole excludes admin at schema layer — BFLA)
  "recitationReadings", // the Recitation Selection on Registration ticket public catalog (static-safe, read-only)
...
| Governed account (deleted/blocked suspended-active login) | generic invalid-credentials `UNAUTHORIZED` (the JWT Authentication Service ticket oracle equality) | — | — | — | — | — |
...
| `app/api/webhooks/whatsapp/route.ts` | Provider webhook | **Formally exempted** per REQ-019 (GET verify + POST 200-ack; HMAC + timingSafeEqual handled by the owning module); correlated logs still required |
...
| `backend/graphql/gqlContextFactory.ts` | MODIFY (minimal) | Add `ctx.requestId` (if the Shared Error Handling & Response Contracts ticket did not land it) and `ctx.idempotencyKey` capture into the returned context — extend in place, no fork (REQ-004). |
...
**Repositories:** none added, none modified. The gateway performs **zero DB access** (REQ-020 purity, REQ-040).
...
| Refresh-rotation race (parallel tabs, stale JTI) | same user, 2 tabs | Divergent session state | **Unchanged by this ticket** — the JWT Authentication Service ticket's stale-JTI honor/rotation contract preserved verbatim; chaos probe re-verifies convergence. |
...
| TOCTOU on anything | — | — | **Null by construction.** The gateway holds no check-then-act state (REQ-041). The only check-then-act in the platform (e.g., registration uniqueness) is enforced by DB constraints upstream (the User Registration ticket pattern), unreachable from gateway code. |
...
- **Visual State Matrix:** None applicable (no rendering). Error rendering contract for consumers of the gateway remains the Shared Error Handling & Response Contracts ticket's errorLink mapping (`FORBIDDEN` → `PermissionDeniedFallback`, validation → RHF fields, etc.) — unchanged by this ticket.
...
10. `login` happy path → response carries **three** `Set-Cookie` headers (`session_id`, `refresh_token`, `access_token`) with the JWT Authentication Service ticket matrix flags.
...
- The gateway performs **zero DB writes**; mass-assignment exposure is structurally impossible at this layer — only consumed domain operations have write paths (their own BOPLA whitelists from the User Registration ticket apply).
...
- `registerUser`'s schema-layer `RegisterPublicRole` gate (excludes `admin`, the User Registration ticket) is unchanged; the gateway does not weaken or bypass it. Transport-tamper variants (raw HTTP crafted roles) still hit the service-layer runtime gate.
...
- No `grantRole*`/`assignRole*`/`elevate*`-shaped mutation exists under any non-admin scope — asserted by the coverage test (REQ-074 preserved).

### 6.4 401-vs-403 Exclusivity (REQ-033)

- `scopeAuth` (no verified context) → `UNAUTHORIZED`; `authScopes` miss (verified context, insufficient role/permission) → `FORBIDDEN`. The gateway performs **no error-code remapping** at the route layer; the Shared Error Handling & Response Contracts ticket finalize boundary passes DomainError codes through untouched. REQ-071(e)/(f) lock the pair behaviorally.
...
- The gateway never constructs queries from user text; no LIKE/ILIKE surface exists here, so `escapeLikeWildcards` is **not applicable** — recorded in the canonical doc so it is not mistaken for an omission. Consumer tickets with search must apply it (forward contract).
...
- `graphql-parse`-time bombs (deep query/circular complexity) are depth-limited at the consumer/configuration level on a later hardening ticket; this ticket records the posture and gates nothing new (deferral is documented, owner: Milestone-4 hardening).

### 6.6 Information Disclosure (REQ-034/036)

- Production: no stack traces, SQL text, parameter values, env names/values, or filesystem paths in any response — enforced by the Shared Error Handling & Response Contracts ticket masking boundary on the GraphQL path and by the envelope helpers on the transport path. The REQ-071(g) probe forces a raw failure and scans the payload for leakage markers.
...
- HTTP-layer per-IP throttling is a **deferral** — ledger entry ⚠️ with owner: production-hardening/Milestone-4 ticket.
...
| `app/api/health/route.ts` via the Shared Error Handling & Response Contracts ticket envelope | REQ-013 |
...
| Allowlist schema-coverage gate (blocking, runs in `test:graphql` + the CI/CD Pipeline ticket `tests` stage) | REQ-072; D3 |
