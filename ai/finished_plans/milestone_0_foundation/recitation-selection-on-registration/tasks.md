...
1. **Pre-Execution:** Before any task, read all files in `ai/plans/recitation-selection-on-registration/outcome/` and this tasks file. Absorb prior pitfalls before editing.
...
4. **Outcome Documentation:** Write `ai/plans/recitation-selection-on-registration/outcome/<task-id>-outcome.md` after each task with files changed, files intentionally not changed, verification results, carry-forward notes, and cross-file dependencies.
5. **Checkbox Tracking:** Mark `[ ]` → `[x]` only after QL/TE/SEC/SR/IV are satisfied. Use `[-]` while in progress.
6. **Deferred Work:** Any blocked schema/persistence decision gets a row in `ai/plans/recitation-selection-on-registration/deferred-items.md` with status ❌/🔄 and owner. No untracked deferrals.
...
- [x] 0.1 Record baseline and initialize ledgers
  ...
  - Create `ai/plans/recitation-selection-on-registration/deferred-items.md` from template.
...
- [x] 5.2 GraphQL and component integration sweep
  ...
  - Execute relevant suites with sanctioned runners: `bun run test:graphql` and targeted run-test for DB logic files.
...
- [x] 6.2 Deferred-items final gate
  - Run `grep -c "❌\|⚠️" ai/plans/recitation-selection-on-registration/deferred-items.md`.
