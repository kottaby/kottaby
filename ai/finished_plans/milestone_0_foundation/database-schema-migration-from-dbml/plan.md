...
**Layer separation note:** all persisted behavior stays inside schema definitions + custom migration SQL. Repositories and services are NOT created by this ticket (beyond entity-setup helpers for tests); business logic belongs to Milestone 1+. `shared/` remains untouched and never imports from `backend/**`.
...
| D5 | No Pothos enum registration, no codegen in this ticket | GraphQL exposure is out of scope for this ticket. Enum registrations land in whichever Milestone ticket first exposes the entity via GraphQL. |
...
- System permission rows, if needed by later Milestone 0 tickets, go in THEIR OWN migration file per the seeds-AGENTS FORBIDDEN rule (not authored by this ticket).

### 2.5 Canonical types (`backend/types/`)

For each new entity: `{Entity}SelectType = typeof tbl.$inferSelect`, `{Entity}InsertType = typeof tbl.$inferInsert` in `backend/types/<domain>/<entity>.types.ts`. Apply `{Entity}ReturnType`/`SubmitInput` ONLY where trivially derivable (omit `deletedAt`); avoid speculative service contracts — Milestone 1 services will extend them. Barrels use `export * from "./..."` relative paths only (`./`, never `@/`), ≤1 slash per export path (root AGENTS.md barrel rules).

---

## 3. API Contracts & Pothos Resolvers

**Not applicable to this ticket.** No GraphQL object types, input types, queries, mutations, or `authScopes` are created. Defensible no-op justification, per the ticket's own scope (schema migration only): the Milestone 0 dependency graph shows GraphQL consumers arrive at the JWT Authentication Service ticket (auth) at the earliest, and exposure happens in Milestone 1+. Pothos enum registration in `backend/graphql/pothos/shared/enum.pothos.ts` and `bun run generate:gqlSchema && bun codegen` are DEFERRED (each lands with the first ticket exposing that entity through GraphQL) and recorded in `deferred-items.md` to prevent lost work.

---

## 4. Backend Services & Repositories

**Services / repositories: not created by this ticket** (data-access conventions already documented in `backend/db/repo/AGENTS.md`; business logic is Milestone 1+ territory).
...
- **Per-Audience Rendering (Student / Parent / Teacher / Supervisor / Admin):** none — audiences consume the schema indirectly through Milestone 1+ UIs.
