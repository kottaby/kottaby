...
**Business value:** The schema is the single most blocking dependency in the roadmap (it gates Milestone 0's M0 release gate and every Milestone 1–4 ticket). A validated, reversible, idempotent migration eliminates schema-drift disputes between streams and unlocks `bun validate:dbml` as a CI gate (the CI/CD Pipeline ticket).
...
- No GraphQL/Pothos resolvers, no Apollo documents, no frontend views or routes (Milestone 0 is schema-only for this ticket).
...
- No repository/service business logic beyond what is needed for constraint enumeration; behavioral flows (booking, escrow, cooldown) are Milestone 1+ tickets.
...
- **REQ-001** (`baseline`): WHEN implementation work begins THEN the executing agent SHALL record baseline `tsgo` / `biome` / `lint-service` error counts and initialize `ai/plans/schema-migration/deferred-items.md` so that new defects are distinguishable from pre-existing ones.
...
|---|---|---|---|---|
| REQ-001 / REQ-002 | — (baseline; `ai/plans/schema-migration/`) | — | — | Plan-review gate + Phase 0 baseline outcome |
