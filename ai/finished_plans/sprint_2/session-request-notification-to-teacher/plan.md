...
> **Plan directory (verbatim — every header, ledger path, and self-reference in this plan uses this exact string):** `ai/plans/sprint_2/session-request-notification-to-teacher`
> **Specs of record:** `ai/plans/sprint_2/session-request-notification-to-teacher/specs.md` (REQ-001..REQ-095)
...
- IF baseline drift is discovered (e.g. -era fields missing from frozen inventories): ledger entry in `ai/plans/sprint_2/session-request-notification-to-teacher/deferred-items.md` (resolved-pointer to the freeze-suite owner) — NEVER a silent re-anchor in this ticket.
...
## Verification Anchors (consumed by tasks.md and `ai/plans/sprint_2/session-request-notification-to-teacher/outcome/`)
...
## Deferred-Items Ledger Pointers (initial content for `ai/plans/sprint_2/session-request-notification-to-teacher/deferred-items.md`)
...
The final gate is `grep -cE '^\s*\|.*(❌|⚠️)' ai/plans/sprint_2/session-request-notification-to-teacher/deferred-items.md` = 0 (row-scoped — the template's Status Values legend lines carry the symbols; only ledger table rows count; plan-review R1 amendment) at completion.
