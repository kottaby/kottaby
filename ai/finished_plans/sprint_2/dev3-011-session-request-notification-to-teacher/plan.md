...
- IF baseline drift is discovered (e.g. -era fields missing from frozen inventories): ledger entry in `ai/plans/sprint_2/dev3-011-session-request-notification-to-teacher/deferred-items.md` (resolved-pointer to the freeze-suite owner) — NEVER a silent re-anchor in this ticket.
...
The final gate is `grep -cE '^\s*\|.*(❌|⚠️)' ai/plans/sprint_2/dev3-011-session-request-notification-to-teacher/deferred-items.md` = 0 (row-scoped — the template's Status Values legend lines carry the symbols; only ledger table rows count; plan-review R1 amendment) at completion.
