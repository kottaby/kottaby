...
> **Plan directory (verbatim — used in every header, ledger path, outcome path, and self-reference below):** `ai/plans/sprint_1/session-report-homework-infrastructure`
> **Specs of record:** `ai/plans/sprint_1/session-report-homework-infrastructure/specs.md`
> **Plan of record:** `ai/plans/sprint_1/session-report-homework-infrastructure/plan.md`
> **Deferred-items ledger:** `ai/plans/sprint_1/session-report-homework-infrastructure/deferred-items.md`
> **Outcome directory:** `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/`
...
1. `ai/plans/sprint_1/session-report-homework-infrastructure/specs.md` and `plan.md` (freshly, not from memory).
2. ALL files already present under `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/` — especially outcomes of prerequisite tasks.
3. `ai/plans/sprint_1/session-report-homework-infrastructure/deferred-items.md` — so previously deferred items are not silently re-implemented or re-deferred without ledger entries.
...
- [x] 0.1 [Record baseline error counts & initialize deferred-items ledger]
  - Run and capture raw outputs + counts into `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/0-baseline-outcome.md`:
  ...
  - Create `ai/plans/sprint_1/session-report-homework-infrastructure/deferred-items.md` from `.agents/spec-process-guide/templates/deferred-items-template.md`.
...
- [x] 0.2 [Prerequisite verification — ground-truth gate before any claim]
  - Verify the following exist in the bundled/working codebase; record `path:line` anchors for each in `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/0.2-outcome.md`. Any MISS downgrades the dependent task's assumption and MUST be reconciled against `plan.md` before proceeding:
...
- [x] 1.1 [Schema verification gate R1–R6 — READ-ONLY, before any schema edit]
  - Read (do not edit) `backend/db/schema/classes/reports.ts` and `backend/db/schema/classes/home-work.ts` bodies and record, with line anchors, in `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/1.1-outcome.md`:
...
- [x] 2.M [Mid-Point Review Gate]
  - Halt implementation; run a consolidated checkpoint BEFORE writing guards/notification/service surface:
    - All Phase 1 tasks and 2.1–2.4 outcomes present in `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/`; all checkboxes of completed tasks flipped with evidence.
  ...
  - Write `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/2.M-midpoint-gate.md` with a GO/NO-GO verdict; NO-GO blocks 2.5+.
...
- [x] 2.9 [Journey test GREEN gate]
  ...
  - Repeat the full journey run 2 more times for flake/fan-out determinism evidence; paste summaries into `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/2.9-outcome.md`.
...
- [x] 5.2 [Differential regression & coverage gate]
  ...
  - Write `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/5.2-outcome.md` with the full matrix (suite → tests passed/failed/skipped counts → deltas).
...
> Waves run as independent review passes. Each appends findings to `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/6-review-waves.md`. Critical findings BLOCK completion until fixed; nits are fixed in-wave or deferred via a NEW ledger row.
...
- [x] 7.3 [Final outcome synthesis & closure]
  - Write `ai/plans/sprint_1/session-report-homework-infrastructure/outcome/final-outcome.md`:
...
4. Ledger `ai/plans/sprint_1/session-report-homework-infrastructure/deferred-items.md` current and reconciled.
