...
- **Ticket**: `docs/planning/TICKETS.md:2838` (Owner Stream: Dev 2, Milestone 4, 3 SP, Blocked By: the audit-emission foundation ticket)
- **Target Directory**: `ai/plans/milestone_4_integration_security_launch/audit-trail-completeness-verification`
- **Outcome Directory**: `ai/plans/milestone_4_integration_security_launch/audit-trail-completeness-verification/outcome`
- **Version**: 1.0
- **Date**: 2026-09-05
...
| G-15 | Audit-emission plan dir | Absent from `ai/plans/milestone_3_parent_portal_admin_governance/` — emission shipped in code without a plan dir | N/A | No blocking reads |
...
#### Acceptance Criteria
1. WHEN implementation begins THEN `bun tsgo`, `bun biome:check`, and `bun run scripts/lint-service.ts --json --id baseline` baselines SHALL be recorded into `ai/plans/milestone_4_integration_security_launch/audit-trail-completeness-verification/outcome/phase0-baseline.md`.
2. WHEN implementation begins THEN the ledger `ai/plans/milestone_4_integration_security_launch/audit-trail-completeness-verification/deferred-items.md` SHALL exist (pre-seeded D-001..D-004 at planning time) and every deferred decision SHALL have a ledger row before its task may close.
3. WHEN an agent starts any task THEN it SHALL read ALL files under `ai/plans/milestone_4_integration_security_launch/audit-trail-completeness-verification/outcome/` first.
