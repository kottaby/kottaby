...
- **Ticket**: `docs/planning/TICKETS.md` §Milestone 4 (Owner Stream: Dev 3, Milestone 4, 5 SP, Blocked By: none)
- **Target Directory**: `ai/plans/milestone_4_integration_security_launch/disaster-recovery-backup-verification`
- **Outcome Directory**: `ai/plans/milestone_4_integration_security_launch/disaster-recovery-backup-verification/outcome`
- **Version**: 1.0
- **Date**: 2026-09-05
...
1. WHEN implementation begins THEN the system SHALL record baseline counts — `bun tsgo 2>&1 | grep -c "error TS"`, `bun biome:check`, and `bun run scripts/lint-service.ts --json --id baseline` — into `ai/plans/milestone_4_integration_security_launch/disaster-recovery-backup-verification/outcome/phase0-baseline.md`.
2. WHEN implementation begins THEN the ledger `ai/plans/milestone_4_integration_security_launch/disaster-recovery-backup-verification/deferred-items.md` SHALL exist (created at planning; D-001..D-003 pre-seeded) and every deferred decision SHALL have a ledger row before its task may close.
3. WHEN an agent starts any task THEN it SHALL read ALL files in `ai/plans/milestone_4_integration_security_launch/disaster-recovery-backup-verification/outcome/` first.
...
#### Additional Details
- **Priority**: High · **Complexity**: Low · **Dependencies**: none · **Assumptions**: quality tooling is green or baselined on milestone_4_integration_security_launch branch.
...
- Milestone 4, 5 SP; `Blocked By: none`; must land before launch-checklist sign-off.
