# Implementation Tasks: Session Request Notification to Teacher

> **Plan directory (verbatim):** `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher`
> **Specs of record:** `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/specs.md` (REQ-001..REQ-095) · `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/plan.md` (D1–D10)
...
1. **Pre-Execution Knowledge Read:** Before executing ANY task, the agent MUST read ALL existing files in `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/` in lexical order. Prior outcomes are binding context — never contradict a ratified ruling without a ledger entry.
...
5. **Outcome Documentation:** Every task writes `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/<task-id>-outcome.md` BEFORE the next task begins.
6. **Checkbox Tracking:** `[ ]` → `[x]` only when ALL subtasks of that task are complete. No partial flips.
7. **Ledger Discipline:** Forward/incomplete-looking items are recorded ONLY as ✅ resolved-pointer entries in `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md`; the final gate is `grep -cE '^\s*\|.*(❌|⚠️)' ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md` = 0 (row-scoped: the template's Status Values legend lines contain the symbols; only ledger table rows count — plan-review R1 amendment).
...
- [x] 0.1 [Record baseline + initialize ledger]
  - Run and capture counts into `/tmp/baseline-*.txt`:
    - `bun tsgo` (type-check error count)
    - `bun biome:check` (warning count)
    - `bun run scripts/lint-service.ts --json --id baseline`
  - Initialize `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md` from `.agents/spec-process-guide/templates/deferred-items-template.md`, seeded with the plan's D1–D7 resolved-pointer entries (intake+accept/decline; B.16 routing; queue persistence; realtime CTA metadata; alternatives computation; freeze-suite baseline drift pointer; caller-tx replay publish posture).
  - Write `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/0-baseline-outcome.md` with the captured numbers (these are the REQ-075 comparator).
...
- [x] 0.3 [@plan-review — ZERO violations required before any implementation task begins]
  ...
  - Write `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/plan-review-R1.md`.
...
- [x] 2.M [Halt and self-audit BEFORE any later phase]
  ...
  - Audit every REQ-001..REQ-053 line against the implemented code; record findings (or clean bill) in `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/2M-midpoint-review-outcome.md`.
...
- [x] 3.1 [Verify zero-GraphQL-drift posture, end to end]
  ...
  - IF any freeze suite fails due to PRE-EXISTING drift (e.g. post- fields absent from frozen inventories): DO NOT re-anchor the baseline — record a ✅ resolved-pointer ledger entry for the freeze-suite owner and document the pre-existing failure with its baseline diff in the outcome.
  - Write `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/3.1-outcome.md`.
...
- [x] 4.1 [Confirm shipped-surface consumption with zero frontend deltas]
  ...
  - Write `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/4.1-outcome.md`.
...
- [x] 5.1 [Full differential gate — engine regression + freeze suites + new suites, all green together]
  ...
  - Evidence (command list + results) written to `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/5.1-outcome.md`.
...
- [x] 5.2 [Baseline gate & final ledger gate]
  ...
  - Final ledger: `grep -cE '^\s*\|.*(❌|⚠️)' ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md` = 0 (row-scoped — only ledger table rows count; plan-review R1 amendment).
  - Schema/migration emptiness proof one final time: `git diff backend/db/schema/** backend/db/migration/**` EMPTY.
  - Write `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/5.2-outcome.md`.
...
- [x] 7.1 [Canonical documentation]
  ...
  - Write `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/7.1-outcome.md`.
...
- [x] 7.2 [AGENTS propagation]
  ...
  - Write `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/7.2-outcome.md`.
  - _Requirements: REQ-082_
- [x] 7.3 [Outcome synthesis & ticket closeout]
  - Synthesize `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/FINAL-outcome.md`: REQ-001→REQ-095 disposition table (met / discharged-by-absence / transferred-as-resolved-pointer), the zero-drift evidence bundle (git diffs, codegen no-diff, freeze-suite greens), the baseline-vs-final lint/type numbers, and links to every task outcome.
  - Final gate re-verification: all checkboxes `[x]`; ledger `grep -cE '^\s*\|.*(❌|⚠️)' ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md` = 0 (row-scoped — plan-review R1 amendment); schema/migration diff empty; engine layer untouched.
