# Technical Architecture & Implementation Design: Session Request Notification to Teacher

> **Plan directory (verbatim — every header, ledger path, and self-reference in this plan uses this exact string):** `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher`
> **Specs of record:** `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/specs.md` (REQ-001..REQ-095)
...
> **Blocking dependency:** (real-time engine) — SHIPPED (`backend/services/notifications/notification-engine.service.ts` — `emitForUser` at:327-369, `publishReceipts` at:475-481, `publishReceiptsFromIndex` at:612-645).
> **Scope ruling under ratification at Phase 1.5:** this ticket ships the canonical six-emitter **notification wave module** ONLY. Session request/accept/decline mechanics, B.16 routing resolution, and any GraphQL/frontend surface are forward consumption contracts — recorded as resolved-pointer deferred entries, never silently absorbed.
...
### 1.1 Scope Statement
 ...
 adds four net-new backend artifacts and nothing else:
...
```text
FUTURE CALLER (session engine — NOT in this ticket)          THIS TICKET
─────────────────────────────────────────────────────────────         ───────────────────────────────
...
| D6 | **Copy keys extend the EXISTING `notifications` namespace** as flat non-`type*` slots (6 titles + 6 body functions + 3 intent labels) instead of a new namespace | (a) new `sessionRequests` namespace; (b) extend `notifications` (D7 precedent) | (a) registry + bundle + parity scaffolding for one emitter family is noise. (b) constraint-compatible: the parity suite pins `type*` keys to exactly the seven enum labels — our keys are `event*`/`intent*`-prefixed, so the invariant survives; the suite's `MANDATED_KEYS`/`FUNCTION_KEYS` inventories are extended in the SAME changeset (REQ-051). | (b). Namespace cohesion + precedent; type-label invariant provably green. |
...
- IF baseline drift is discovered (e.g. -era fields missing from frozen inventories): ledger entry in `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md` (resolved-pointer to the freeze-suite owner) — NEVER a silent re-anchor in this ticket.

### 3.2 Permission Matrix (no new surface — behavior is inherited)

| Caller | Behavior on this ticket's emission surface |
|---|---|
| Anonymous | No GraphQL surface exists to call. Emitters are service-internal; no anonymous path exists by construction (a `sessionId` is only meaningful to a caller who already owns a committed session row — the future scope-gated intake). |
...
3. row = await SessionRepository.findWaveContextById(sessionId, tx)
   row === null → ONE logDomainError("Session not found for notification wave", { code:"SESSION_NOT_FOUND", entity:"session", entityId, locale })
                  → throw NotFoundError("SESSION", tErrors.sessionNotFound)
   (message-first shape per backend/lib/logger.ts:92; null joined read ⇒ session absent — the unreachable-participant INTERNAL_SERVER_ERROR branch is dropped per plan-review R1)
4. intent guard is NULL-FIRST: `if (row.intent === null || !isSessionIntent(row.intent))` ⇒ corrupt
   (D7: null ⇒ corrupt; otherwise `row.intent` is narrowed to SessionIntent; the guard file is never edited — plan-review R1 amendment)
   corrupt → ONE logDomainError("Session intent corrupt for notification wave", { code:"SESSION_INTENT_CORRUPT", entity:"session", entityId, locale })
...
9. if (tx !== undefined) {
     ...
     return result;   // caller-tx: the engine ALWAYS returns the receipt on this path — returned verbatim; NO publish
     // (plan-review R1 amendment: under caller-tx "notifications" in result is trivially true — the old wrap fallback was dead code)
   }
...
- Logging: exactly ONE `logger.logDomainError(<pinned message>, { ... })` per expected rejection — message-first shape (`backend/lib/logger.ts:92`: `logDomainError(message: string, ctx?)`; plan-review R1 amendment), pinned messages "Session not found for notification wave" / "Session intent corrupt for notification wave", bounded context `{ code, entity: "session", entityId: sessionId, locale }` — never copy, never participant names, never raw idempotency keys (REQ-033/052). Happy paths log NOTHING (REQ-053).
...
```mermaid
stateDiagram-v2
[*] --> Recorded: System commits session row (engine-owned intake is)
...
## Verification Anchors (consumed by tasks.md and `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/outcome/`)
...
7. **Baseline gate (REQ-075):** `bun tsgo` / `bun biome:check` / lint counts ≡ REQ-001 baseline + ZERO new errors; EVERY created/modified file passes `bun run scripts/health/sub-loop.ts <file> --lifecycle duplicates` exit 0; final ledger gate `grep -cE '^\s*\|.*(❌|⚠️)' ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md` = 0 (row-scoped per plan-review R1 amendment).
8. **Docs (REQ-080..082):** NEW canonical doc `docs/notifications/session-request-notifications.md` (Why → six-wave Pattern + key/entity conventions → Rules: single-writer, caller-tx receipt / publish-after-commit, recipient-locale composition, closed payload → What NOT to Do → Rollout file table → Forward Consumption Contract → Related Documents); `docs/notifications/realtime-engine.md` §3.2 row gains the one-line shipped pointer; `backend/services/AGENTS.md` + `backend/db/repo/AGENTS.md` gain their one-liners; root `AGENTS.md` Important References gains the doc line. `docs/specs/*` remain UNEDITED (no new invariants minted).

## Deferred-Items Ledger Pointers (initial content for `ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md`)

| ID | Item | Owning direction | Status at plan time |
|---|---|---|---|
| D1 | Session intake + accept/decline mutations + session-row authorship (the ONLY writer of `session` rows) |  | ✅ resolved-pointer |
| D2 | B.16 ROUTE resolution (in-session detection + preference → which outcome wave to fire) |  (availability) +  (matching) +  | ✅ resolved-pointer |
| D3 | Queue persistence for the `queue` preference (NO pending-request entity exists in schema) | session-engine design (era) | ✅ resolved-pointer |
| D4 | Actionable accept/decline CTA metadata on the realtime payload (engine-owned projection widening) |  lineage / session engine's UI ticket | ✅ resolved-pointer |
| D5 | Alternative-teacher computation for `offer_alternatives` (matching engine surplus) |  | ✅ resolved-pointer |
| D6 | Any discovered freeze-suite baseline drift (e.g. pre- inventories) | freeze-suite owner ticket | ✅ resolved-pointer |
...
The final gate is `grep -cE '^\s*\|.*(❌|⚠️)' ai/plans/milestone_2_matching_notifications_escrow/session-request-notification-to-teacher/deferred-items.md` = 0 (row-scoped — the template's Status Values legend lines carry the symbols; only ledger table rows count; plan-review R1 amendment) at completion.

---

**Governing next step:** Phase 1.5 — run `@plan-review` on the complete plan (`specs.md` + `plan.md` + tasks) BEFORE any implementation; the two rulings it MUST ratify are (a) the §0 scope reconciliation (notification-wave-only ticket; engine machinery stays) and (b) the D2 signature reconciliation (the `locale` second positional parameter absent from specs.md REQ-011's literal shape).
