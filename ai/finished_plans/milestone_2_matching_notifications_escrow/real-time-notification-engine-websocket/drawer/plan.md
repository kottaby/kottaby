...
**Parent plan:** `ai/plans/milestone_2_matching_notifications_escrow/real-time-notification-engine-websocket/` (implemented)
...
2. `bun quality-gate` (tsgo → oxlint → biome → lint → duplicates). The hook extraction must keep
   `duplicates` green (that is its purpose) — no `jscpd:ignore`.
3. Optional manual browser check via `agent-browser` (snapshot/text assertions first, screenshots to
