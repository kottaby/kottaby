...
> `ai/plans/realtime-notification-engine/` (and its `outcome/` / `deferred-items.md` sub-paths) in
> specs.md / plan.md / this file means the ACTUAL plan directory:
> **`ai/plans/milestone_2_matching_notifications_escrow/real-time-notification-engine-websocket/`**. All outcome files, the
> deferred-items ledger, and the REQ-083 final grep gate use the milestone_2_matching_notifications_escrow path. See
...
5. **Outcome Documentation**: Every task writes `ai/plans/realtime-notification-engine/outcome/<task-id>-outcome.md` recording: what was built, verification commands + exit codes, coverage evidence, deferred items touched, baseline delta.
...
- [x] 0.1 [Record error baseline and initialize the deferred-items ledger]
  ...
  - Create `ai/plans/realtime-notification-engine/deferred-items.md` from the template; pre-seed as non-blocking forward items:
...
- [x] 6.2 [Deferred-items gate]
  - `grep -c "❌\|⚠️" ai/plans/realtime-notification-engine/deferred-items.md` equals 0 EXCLUDING the pre-seeded forward items D1–D4 (each with owning ticket recorded, non-blocking)
...
- [x] 7.4 [Close the plan]
  ...
  - Final verification bundle re-run and recorded: baseline delta = 0; schema drift empty; codegen artifacts committed; all suites (`backend/db/test/logic/notifications`, engine services, WS, GraphQL integration, chaos, `bun test test/workflows`) green across two consecutive runs
