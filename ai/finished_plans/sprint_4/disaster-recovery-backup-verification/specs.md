...
1. WHEN implementation begins THEN the system SHALL record baseline counts — `bun tsgo 2>&1 | grep -c "error TS"`, `bun biome:check`, and `bun run scripts/lint-service.ts --json --id baseline` — into `ai/plans/sprint_4/disaster-recovery-backup-verification/outcome/phase0-baseline.md`.
2. WHEN implementation begins THEN the ledger `ai/plans/sprint_4/disaster-recovery-backup-verification/deferred-items.md` SHALL exist (created at planning; D-001..D-003 pre-seeded) and every deferred decision SHALL have a ledger row before its task may close.
3. WHEN an agent starts any task THEN it SHALL read ALL files in `ai/plans/sprint_4/disaster-recovery-backup-verification/outcome/` first.
