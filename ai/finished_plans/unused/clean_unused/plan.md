...
- NEVER run UI or E2E tests — no Storybook, Vitest browser runs, Playwright, or `build:test`. Not for verification, not for spot-checks, not ever. Verification is limited to `check:unused`, `tsgo`, lint, and the quality-gate/quality-loop scripts.
