...
> **Plan directory (verbatim):** `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ`
> **Ticket:** Broadcast Notifications (System-Wide & Targeted) · Dev 3 · Milestone 3 · 3 SP · Blocked (shipped)
...
- **REQ-074 (Frontend Document & Namespace Tests):** THEN the shared-document test SHALL pin operation name/variable surface/Int payload (pattern `notification.documents.test.ts`), the NEW `adminBroadcasts-namespace.parity.test.ts` SHALL pin ar/en key parity + plural-function output, and the errors-namespace parity SHALL keep passing with the four new keys present in both locales.
...
| REQ-063/064/065 | UX/i18n/MUI rules | — | — | `/admin/broadcasts` page + container + nav item | `navItems.test.ts` extension |
| REQ-070..076 | test-layer rules | repo/service suites | integration matrix | — | 4-tier + coverage + baseline-zero drift |
