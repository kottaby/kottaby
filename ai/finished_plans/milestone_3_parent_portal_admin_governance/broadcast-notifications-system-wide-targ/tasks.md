...
> **Plan directory (verbatim — used everywhere below):** `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ`
> **Specs:** `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/specs.md` (REQ-001..REQ-082, §2.9 journeys, DB-1..DB-6)
> **Plan:** `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/plan.md`
> **Outcomes:** `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/outcome/`
> **Deferred items:** `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/deferred-items.md`

---

## Non-Negotiable Execution Protocol

1. **Pre-Execution Outcome Knowledge Read:** before starting ANY task, read ALL existing files under `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/outcome/` and `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/deferred-items.md`. Never repeat a resolved mistake; never redo finished work.
...
5. **Outcome Documentation:** after each task, write `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/outcome/<task-id>-outcome.md` capturing: what changed (files), verification evidence (command outputs), deviations, follow-ups.
...
- [x] 0.1 [Record error baseline + initialize deferred-items ledger]
  ...
  - Create `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/deferred-items.md` from `.agents/spec-process-guide/templates/deferred-items-template.md`; seed it with plan ledger entries: **D1** chunked mega-broadcast (>5000 cohorts) → future scale ticket; **D2** crash-between-commit-and-`publishReceipts` double-insert residual (engine §3.6 document-locked posture) → engine hardening stream.
  - Write `ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/outcome/0.1-baseline-outcome.md` with counts + snapshot.
...
- [x] 4.2 [Create `/admin/broadcasts` route — Server Component guard]
  ...
  - [x] 4.2.TE **Unit / Component Tests:** N/A — server-guard behavior is the SSR `withPageAuth` gate; covered by the 4.2.SEC audit (anonymous → `/login?redirect=/admin/broadcasts`).
...
- [x] 4.3 [Create `BroadcastComposeContainer` — compose UI] (DISPOSITION: QL/SR/IV green — see outcome/4.3-outcome.md)
  ...
  - [x] 4.3.BS **Agent-Browser Visual & Styling Self-Loop (Screenshot Analysis)** — CLOSED: browser-QA pass executed via commit 3772f36 (10/10 responsive polish) + `prototype/` artifacts; remaining runtime execution → DF-2:
...
- [x] 5.2 [Baseline gates & drift checks (REQ-076, REQ-044)]
  ...
  - `grep -c "❌\|⚠️" ai/plans/milestone_3_parent_portal_admin_governance/broadcast-notifications-system-wide-targ/deferred-items.md` MUST be 0 (D1/D2 retargeted with owners, not blockers).
