# Trackable Implementation Tasks — Account Soft-Delete Governance (users.is_deleted)

# tasks.md — Account Soft-Delete Governance (users.is_deleted)

> **Plan directory (verbatim — every header, ledger path, outcome path, and self-reference in this document uses this exact string):** `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d`
> **Specs of record:** `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/specs.md` (REQ-001..REQ-095)
> **Plan of record:** `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/plan.md` (D1..D12)
...
1. **Pre-Execution Outcome Knowledge Read (MANDATORY):** BEFORE starting any task, read ALL existing files under `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/outcome/`. Incorporate every prior decision, discovered hazard, baseline count, and ledger entry into the current task's execution. Never re-litigate a resolved item; never contradict a recorded ruling without a new ledger entry.
...
5. **Outcome Documentation (MANDATORY):** after completing a task, write `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/outcome/<task-id>-outcome.md` capturing: what was done, files touched, verification evidence (commands + results), deviations, hazards discovered, and ledger updates.
...
- [x] 0.1 [Record baseline error counts & initialize deferred-items ledger]
  ...
  - Initialize `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/deferred-items.md` from `.agents/spec-process-guide/templates/deferred-items-template.md`, PRE-SEEDED with the seven resolved-pointer rows from plan.md §Deferred-Items Ledger Pointers (D1 lapse sweep, D2 session-creation predicate consumption, D3 governance-notification, D4 strict-guard backport ownership, D5 context-boundary governance gate, D6 audit_vocabulary widening, D7 SSR test seam) — ALL as resolved-pointer status, ZERO ❌/⚠️ markers.
  - Write `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/outcome/0-baseline-outcome.md` with counts, diff set, and ledger-initialization confirmation.
...
- [x] 0.2 [Verify reuse substrate & conditional-shape probes (Reuse-Not-Rebuild guard)]
  ...
  - IF any reuse artifact is missing → record a ❌ ledger entry in `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/deferred-items.md` and BLOCK dependent tasks — never fork a second writer/guard/transition engine.
  - _Requirements: REQ-004, REQ-020, REQ-061_

- [x] 0.3 [Phase 1.5 Plan-Review Gate]
  - Invoke `@plan-review` over the complete plan trio (`specs.md`, `plan.md`, `tasks.md`) for `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/`.
  - Resolve ALL findings (no silent skips); iterate until the review passes clean.
  - Write `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/outcome/plan-review-R1.md` with the verdict, findings, and resolutions.
...
- [x] 2.1 [Write account-governance journey test — TEST-FIRST (before any service surface)]
  ...
  - Sequential actor-attributed steps (each = service call with `actorUserId` → shared-state assertion → cross-actor visibility assertion → side-effect oracles):
    ...
    7. A soft-deletes S via the EXISTING `setUserDeleted` (consumed, never forked) → login denied; A then attempts suspend on DELETED S → `USER_ALREADY_DELETED` + ZERO new audit rows; A reactivates S (existing path) → login succeeds (full lifecycle loop).
...
- [x] 2.2 [Create-or-consume shared admin guard module + strict window-aware variant]
  ...
  - **Branch A (ABSENT — sibling not landed):** CREATE the module via BEHAVIOR-PRESERVING extraction of the private `assertActorAdmin` (`user-management.service.ts:240-271`) — identical behavior; delete the private copy from `user-management.service.ts` and import the helper (the EXISTING methods keep RELAXED semantics — REQ-031; their existing suites are the byte-equivalence net). ADD:
    ```typescript
...
- [x] 2.4 [Extend `AdminUserManagementService` — `setUserSuspended` / `setUserBlocked`]
  ...
  - [ ] 2.4.TE **Test Engineering — 4-Tier**: NEW `backend/services/admin/user-governance.service.test.ts`:
    ...
    - Run via `bun run test/scripts/run-test.ts backend/services/admin/user-governance.service.test.ts`; ALSO re-run the EXISTING user-management suite — MUST stay byte-green with ZERO edits (REQ-020).
...
- [x] 4.2 [i18n — `AdminUsers.governanceActions` group + `detail.governanceNote` copy fix, both locales]
  ...
  - [ ] 4.2.TE **Test Engineering**: existing translation-parity suites green; the `AdminUsers` handle exposes the new group (compile-time proof).
...
- [x] 5.1 [Full-suite integration run + regression nets]
  ...
  - Write `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/outcome/5-integration-outcome.md` with the full matrix of commands + results.
  - _Requirements: REQ-020, REQ-070, REQ-071, REQ-072, REQ-073, REQ-075_
...
- [x] 5.2 [Static locks & INV-U4 grep-lock suite]
  ...
  - Final ledger gate: `grep -c "❌\|⚠️" ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/deferred-items.md` = 0 (all forward items remain resolved-pointer rows D1-D7).
  - Instruction files: `.agents/instructions/tests.instructions.md`, `.agents/instructions/backend.instructions.md`.
...
> Launch the four review wave agents IN PARALLEL over the full changeset; each writes its finding set into `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/outcome/`. Resolve EVERY finding (fix or ledger-justified rejection) before Phase 7.
...
- [x] 6.5 [Deferred-items cross-check] — re-read `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/deferred-items.md`; confirm ZERO ❌/⚠️; confirm resolved-pointer rows D1-D7 intact and referenced (never silently absorbed): lapsed-suspension sweep; session-creation predicate consumption; notification-on-governance; strict-guard backport ownership; context-boundary gate; audit vocabulary widening; SSR test seam. Output appended to the integration outcome.
...
- [x] 7.2 [Inbound/outbound doc reconciliation pointers]
  - `docs/admin/user-management.md` §6 scope-split row for → flip to shipped (ONE line pointer; NO renumbering, NO re-litigating JR-C-1).
...
- [x] 7.4 [Outcome synthesis & final gates]
  - Write `ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/outcome/7-completion-outcome.md`: synthesis of ALL task outcomes; final verification table (every plan.md Verification Anchor 1-12 with command + result evidence); baseline diff = 0 attestation; zero-drift `git diff` attestation; codegen artifacts committed attestation; journey green attestation; final `grep -c "❌\|⚠️" ai/plans/milestone_3_parent_portal_admin_governance/account-soft-delete-governance-usersis-d/deferred-items.md` = 0 proof.
...
| 4.1 | REQ-062 |
| 4.2 | REQ-002, REQ-064 |
| 4.3 | REQ-002, REQ-063, REQ-064, REQ-065 |
| 5.1 | REQ-020, REQ-070..073, REQ-075 |
| 5.2 | REQ-045, REQ-075 |
| 6.1–6.5 | REQ-075, REQ-083 |
| 7.1–7.4 | REQ-080, REQ-081, REQ-082, REQ-083, REQ-075 |

**End of tasks.md.** Execution begins at Phase 0; the Phase 1.5 plan-review gate (task 0.3) MUST pass before any implementation task starts.
