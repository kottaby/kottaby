# Technical Architecture & Implementation Design: Cold-Start Bootstrapping (Direct Sheikh Certification)

> **Plan of record:** `ai/plans/milestone_3_parent_portal_admin_governance/cold-start-bootstrapping-direct-sheikh-c/`
> **Specs:** `ai/plans/milestone_3_parent_portal_admin_governance/cold-start-bootstrapping-direct-sheikh-c/specs.md` (REQ-001..REQ-083, journey J-1)
> **Canonical refs:** `docs/admin/user-management.md` (substrate + scope-split), `docs/teachers/applicant-lifecycle.md` (lifecycle finalize rule), `docs/workflows/05-admin-governance-override.md` §3 (owner workflow), `docs/specs/open-decisions-and-gaps.md` (A.4/A.4.3, A.5, A.7, B.6/B.7), `docs/specs/state-machine-invariants.md` (INV-TV1..TV7, INV-U1..U5), `docs/notifications/realtime-engine.md` (single-writer + publish-after-commit), `docs/graphql/api-gateway-and-routing.md`, `docs/graphql/error-handling-contract.md`, `docs/testing/workflow-journey-tests.md`, `test/workflows/AGENTS.md`
> **Blocking dependency:** (Admin User Management — SHIPPED; verified in-tree)
> **Ledger:** `ai/plans/milestone_3_parent_portal_admin_governance/cold-start-bootstrapping-direct-sheikh-c/deferred-items.md` (initialized in Task 0)
...
3. **`admin-gate.helpers.ts`** (NEW shared service helper — extracted `assertActorAdmin` + NEW governance-extended `assertActorAdminActive`; first-lander rule per the REQ-004 extraction-collision note).
...
| D10 | **Repeat-call safety = conflict, not idempotency keys** | (a) wire `ctx.idempotencyKey`; (b) conflict-answers-replay | This mutation is OUTSIDE the mandated `docs/IDEMPOTENCY.md` key set; the second call structurally resolves to `TEACHER_ALREADY_CERTIFIED` BEFORE any write/notify (REQ-043), mirroring the admin-ops idempotency ruling (`docs/admin/user-management.md` §2.5) | REQ-043. No claim cache, no key derivation, no replay machinery |
...
**Completion gate:** `git diff -- backend/db/schema/** backend/db/migration/**` MUST be EMPTY. No `bun run db push`, no new `pgEnum`, no custom SQL migration (the audit immutability-trigger question belongs to the REQ-018, NOT this ticket).
...
```mermaid
stateDiagram-v2
[*] --> ApplicantNoTeacherRow: admin createUser (teacher role) — substrate
...
| Certified teacher (post-certification) | observes ONE `evaluation_result` notification in the EXISTING inbox surface (badge/drawer — zero recipient-side UI changes; substrate consumes the new rows unchanged) |
...
| **PII / content hygiene** | Audit `details` = fixed 3-field JSON (flags only — never names/emails/content, D8); actor disclosure on the trail remains `actorId`+actor-name resolution as. Denial messages come from compile-time i18n only; no error path discloses anything beyond the closed code set. |
...
## Deferred items (pre-registered in `ai/plans/milestone_3_parent_portal_admin_governance/cold-start-bootstrapping-direct-sheikh-c/deferred-items.md` at Task 0 — resolved-as-reference entries only)
