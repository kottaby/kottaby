...
**Specs:** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/specs.md`
**Plan:** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/plan.md`
**Deferred-items ledger:** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/deferred-items.md`
...
- Outcome files are named `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/<task-id>-outcome.md`.


## Non-Negotiable Execution Protocol

1. **Pre-Execution outcome knowledge read:** Before starting ANY task, read the outcome files of every task it depends on under `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/` (esp. `0.1-outcome.md` and `0.2-outcome.md`). If a dependency outcome is missing or marked ❌/⚠️, STOP and resolve it first.
...
5. **Outcome documentation:** Every task writes `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/<task-id>-outcome.md` recording: what was verified/changed, evidence (exit codes, test counts, diffs), deviations, and any ledger entries added.
...
- [x] 0.1 Record the pre-implementation error baseline and initialize the ledger
  - Run `bun x tsgo --noEmit` (or the repo's typecheck script), `bun run biome:check`, and the lint-service check; record exact error/warning counts per tool in `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/0.1-outcome.md`.
  - Create `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/deferred-items.md` from `.agents/spec-process-guide/templates/deferred-items-template.md` (empty body, headers intact).
...
- [x] 2.2 Audit existing service/repo suites against acceptance criteria; add ONLY genuinely missing regression cells
  ...
  - Add ONLY missing cells (expected candidates; each must be justified by the diff audit — no duplicates of cells):
    ...
    - Governance pre-tx ordering: `requireActor` fires BEFORE any transaction opens (assert via repo-spy zero invocations on governed actor) — REQ-022 D9a.
...
- [x] 4.1 Verify/close the notification-drawer deep-link for `parent_link_request` (REQ-011)
  ...
  - **If the `relatedEntityType === "parent_link_request"` branch already routes to the student decision route:** PIN it with the route-resolution contract test only — no code edit.
  ...
  - Cover via the route-resolution contract test: a `parent_link_request` notification resolves exactly to the student decision route (both locales; no router errors for unknown entity types — those fall through unchanged).
  ...
  - [x] 4.1.TE **Unit / Component Tests:** route-resolution contract tests (`frontend/lib/notification-route-resolution.test.ts`): notification click → route resolution; unknown entity → unchanged behavior; both locales.
...
- [x] 4.5 Pin the decision page's existing behavior (regression, no redesign)
  - Run the existing `parent-link.documents.test.ts` and adjacent surviving suites — all green, unmodified.
...
- [x] 5.1 Full integration battery & differential verification
  - Run (each via `bun run test/scripts/run-test.ts <path>`):
    ...
    4. nav tests + drawer deep-link tests + dashboard-home slot tests
  ...
  - _Requirements: REQ-050, REQ-060, REQ-061, REQ-063, REQ-065_
...
> Launch the four review waves in parallel; each produces a findings file under `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/`. All Critical/High findings must be resolved (or ledgered with explicit rationale) before Phase 7.
...
- [x] 6.5 Deferred-items ledger gate (FINAL)
  - Re-read `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/deferred-items.md` in full.
...
|---|---|
| Student sees pending link request (list + notification + dashboard) | 1.1, 4.1, 4.2, 4.3 + journey steps 1–2 (Task 2.1) |
...
- [ ] Read ALL files in `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/` (prior findings; do not re-research)
