# Technical Architecture & Implementation Design: Student Confirmation of Parent Link

**Plan directory (verbatim):** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link`
**Specs (approved):** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/specs.md`
**Deferred-items ledger (to be created at Task 0):** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/deferred-items.md`
**Outcome directory:** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/`
**Governing docs:** `docs/parents/parent-link-request.md` (canonical, status "Implemented and verified"), `docs/parents/handshake-code-discovery.md`, `docs/workflows/04-parent-supervision-handshake.md` (§4.4), `docs/specs/state-machine-invariants.md` (INV-P1..P4), `docs/specs/open-decisions-and-gaps.md` (A.2/A.3/A.4/B.12/B.13/B.14), `docs/notifications/realtime-engine.md` (§3.1/3.2), `docs/testing/workflow-journey-tests.md`.
...
- **Target Directory**: `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/`
- **Outcome Directory**: `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/outcome/`
...
- **Related Documents**: `specs.md` (requirements), `tasks.md` (execution), `docs/parents/parent-link-request.md` (canonical domain doc), `ai/plans/milestone_3_parent_portal_admin_governance/*/` (substrate outcome)
...
### 1.1 What this ticket is (and is not)
 ...
 is the **student-side decision half** of Workflow 04 §4.4, shipped as a **closure + discoverability slice on top of the already-verified substrate**. The bundled code confirms the entire authorizing stack exists:
...
```text
Parent (surfaces)                  Student (this ticket)                 Parent (observer)
─────────────────────────                   ──────────────────────────            ──────────────────
requestLink(code) ──┐
                    ▼
...
| `myOutgoingParentLinkRequests` / `requestParentChildLink` / `cancelParentLinkRequest` (context) | 401 | 403 | n/a | 200 (parent ops) | 403 | 403 |
...
| Symbol | File | Signature (bundled) | Role in  |
|---|---|---|---|
...
| `requireActor` | same | fresh actor re-check (identity+role+governance, constant-copy governed denials) | REQ-022 (invoked BEFORE the tx opens per the recorded D9a divergence) |
...
**Shared-entity state machine** (`parent_link_requests` row — the vocabulary, unchanged):

```mermaid
stateDiagram-v2
    [*] --> pending: Parent creates request 
...
|---|---|---|---|
| → pending | Parent (`UserRole.Parent`, POST-create surface) | pair arbiter + discovery/governance collapse | student/teacher/admin |
...
| Student link-requests route (**verify at implementation**: locate via `rg "myIncomingParentLinkRequests" app/ frontend/views/students/` — expected an `app/(dashboard)/student/**` page rendering the `frontend/views/students/link-requests/**` container) | Decision surface (list + Confirm/Reject) | authenticated + role | Student | same |
...
| Parent | No slice of this surface (their faces are the outgoing list/notifications) |
...
- **Cache convergence (REQ-016):** after Confirm/Reject the decision page performs id-first write-back + list refetch (behavior); the card disappears automatically on its next `useQuery` evaluation after document-level cache update / drawer badge refresh — no bespoke invalidation bus.
...
Automated real-browser pass (the D8-compensating control; Happy-DOM portal-input limits apply to dialog textareas only; this surface has none):
...
4. Repeat with **Confirm** on a fresh pending → `students.parent_id` set (assert via parent outgoing view state + subsequent portal eligibility, read-only), sibling pendings (seed two parents) render `Expired`, card disappears.
...
- **INV-P1 closure (REQ-017/065):** negative probes assert `students.parent_id` is NULL before confirmation, unchanged after rejection, and that no parent-side read of student data exists anywhere in this surface (parent portal is the scope and reads ONLY `students.parent_id`).
...
| Notification deep-link | contract test on the drawer resolution | REQ-011 pin (post-verification) |
...
| Docs | amend `docs/parents/parent-link-request.md` (closure section) | REQ-070 — no new canonical doc, no edits to spec/invariant numbering |
...
- **Interfaces**: Props — none (self-contained hook container). Apollo document — `MyIncomingLinkRequestsQueryDocument` + mutation document (both pre-pinned; verify).
...
- **Backward compatibility**: additive-only GraphQL surface; no breaking input changes; existing consumers unaffected.
