...
**Specs file:** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/specs.md`
**Deferred-items ledger (to be created):** `ai/plans/milestone_3_parent_portal_admin_governance/student-confirmation-of-parent-link/deferred-items.md`
...
- **Ticket Reference**: `` in `docs/planning/TICKETS.md` (Milestone 3, `Blocked By: (satisfied)`, `Blocks: `)
...
| REQ-011 (notification + deep link) | A.4 | `emitRequestNotificationTx` (substrate) | `myNotifications` / drawer documents | `frontend/components/ui/NotificationDrawer*.tsx` deep-link behavior | Contract test pinning `parent_link_request` route resolution |
...
| REQ-015 (dashboard card — NEW) | B.14, INV-P1 | (client `useQuery` over existing service) | `myIncomingParentLinkRequests` | NEW card under `frontend/views/**` (student dashboard home, alongside `HandshakeCodeCard` slot in `RoleDashboardPage`) | — |
| REQ-016 (post-decision convergence) | — | — | id-first mutation document write-back | Card disappearance + list refetch | Journey assertion |
...
| Term | Definition |
|---|---|
| ** substrate** | The service/repo/resolver/notification spine for parent link requests, implemented ahead of per milestone order. |
