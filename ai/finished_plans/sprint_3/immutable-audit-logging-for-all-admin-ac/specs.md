...
> **Ticket:** (Sprint 3 · Dev 3 · 5 SP · blocked: DONE)
> **Plan directory (verbatim — every header, ledger path, task 0.1, and deferred self-reference MUST use this exact string):** `ai/plans/sprint_3/immutable-audit-logging-for-all-admin-ac`
...
> **Ledger (initialized in Task 0):** `ai/plans/sprint_3/immutable-audit-logging-for-all-admin-ac/deferred-items.md`
>
...
- **REQ-001 (Pre-Implementation Baseline & Ledger):** WHEN implementation begins THEN the executing agent SHALL record baseline error counts (`bun tsgo`, `bun run oxlint`, `bun biome:check`, `bun run lint --json --id baseline`) AND initialize `ai/plans/sprint_3/immutable-audit-logging-for-all-admin-ac/deferred-items.md` from `.agents/spec-process-guide/templates/deferred-items-template.md` AND write `ai/plans/sprint_3/immutable-audit-logging-for-all-admin-ac/outcome/phase0-baseline-outcome.md` capturing counts and the pre-existing modified-file set (`git diff --name-only`).
...
- **REQ-004 (Verification-First Reuse Guard — verify-then-claim):** WHEN domain work begins THEN the agent SHALL verify against the bundled/live code and REUSE (never fork):
  ...
  - IF any required artifact is missing THEN record a ❌ entry in `ai/plans/sprint_3/immutable-audit-logging-for-all-admin-ac/deferred-items.md` and block the dependent task — never inline-patch a foreign layer.
...
- **REQ-082 (Outcome Protocol):** every task SHALL read prior files in `ai/plans/sprint_3/immutable-audit-logging-for-all-admin-ac/outcome/` first and write `outcome/<task-id>-outcome.md` after; the final gate SHALL re-verify baseline deltas AND `grep -c "❌\|⚠️" ai/plans/sprint_3/immutable-audit-logging-for-all-admin-ac/deferred-items.md` = 0.
