...
> **Plan directory (verbatim — every header, ledger path, outcome path, and self-reference in this file MUST use this exact string):** `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c`
> **Specs:** `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/specs.md`
> **Plan:** `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/plan.md`
> **Ledger:** `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/deferred-items.md`
> **Outcomes:** `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/`
...
1. **Pre-Execution Outcome Read:** Before starting any task, read ALL existing files under `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/` to accumulate cross-task knowledge (files changed, deliberately-NOT-changed files, pitfalls, verified line anchors).
...
5. **Outcome Documentation:** After completing each task, write `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/<task-id>-outcome.md` capturing: summary, files changed + deliberately-NOT-changed, verification outputs (exact commands + results), cross-file dependencies discovered, and deviations from plan (if any — each deviation MUST be justified or reverted).
6. **Checkbox Tracking:** Flip `[ ]` → `[x]` on the task AND every completed subtask as work lands. Never batch-flip at the end.
7. **Verification-First Ground Truth:** If any cited symbol/file/line is not locatable in the bundle, STOP, record a ❌ entry in `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/deferred-items.md`, and re-scope — never invent code to match docs prose.
...
- [x] 0.2 [Prerequisite verification — substrate existence audit (verify-then-claim)]
  ...
  - Write `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/0.2-outcome.md`.
  - [x] 0.2.QL **Quality Loop**: `bun run scripts/health/sub-loop.ts ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/0.2-outcome.md --lifecycle duplicates` (exit 0)
...
- [x] 1.9 [PHASE 1.5 — `@plan-review` gate (MANDATORY before ANY implementation task below)]
  - Invoke `@plan-review` over `specs.md` + `plan.md` + this `tasks.md`; record the verdict in `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/plan-review-R1.md`.
...
- [x] 2.M [PHASE 2.M — Mid-Point Review Gate (MANDATORY)]
  ...
  - GATE: any drift ⇒ fix before Phase 3; record verdict in `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/2M-midpoint-gate-outcome.md`.
...
- [x] 5.3 [End-state gates — baseline delta, zero-drift, ledger, hygiene]
  ...
  - `grep -c "❌\|⚠️" ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/deferred-items.md` MUST equal `0` (all D-* entries RESOLVED-REFERENCE with owning tickets).
  - `console.*` sweep over every created/modified file MUST be empty.
  - Record everything in `ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/outcome/5.3-endgates-outcome.md`.
...
- [x] 6.5 [Reconciliation + deferred-items final check]
  ...
  - Final ledger gate: `grep -c "❌\|⚠️" ai/plans/sprint_3/cold-start-bootstrapping-direct-sheikh-c/deferred-items.md` = 0.
