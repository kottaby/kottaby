...
> **Plan directory (verbatim — every header, ledger path, outcome path, and self-reference in this document uses this exact string):** `ai/plans/sprint_3/account-soft-delete-governance-usersis-d`
> **Specs of record:** `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/specs.md` (REQ-001..REQ-095)
> **Plan of record:** `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/plan.md` (D1..D12)
...
1. **Pre-Execution Outcome Knowledge Read (MANDATORY):** BEFORE starting any task, read ALL existing files under `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/outcome/`. Incorporate every prior decision, discovered hazard, baseline count, and ledger entry into the current task's execution. Never re-litigate a resolved item; never contradict a recorded ruling without a new ledger entry.
...
5. **Outcome Documentation (MANDATORY):** after completing a task, write `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/outcome/<task-id>-outcome.md` capturing: what was done, files touched, verification evidence (commands + results), deviations, hazards discovered, and ledger updates.
...
- [x] 0.2 [Verify reuse substrate & conditional-shape probes (Reuse-Not-Rebuild guard)]
  ...
  - IF any reuse artifact is missing → record a ❌ ledger entry in `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/deferred-items.md` and BLOCK dependent tasks — never fork a second writer/guard/transition engine.
  - _Requirements: REQ-004, REQ-020, REQ-061_

- [x] 0.3 [Phase 1.5 Plan-Review Gate]
  - Invoke `@plan-review` over the complete plan trio (`specs.md`, `plan.md`, `tasks.md`) for `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/`.
  - Resolve ALL findings (no silent skips); iterate until the review passes clean.
  - Write `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/outcome/plan-review-R1.md` with the verdict, findings, and resolutions.
...
- [x] 5.1 [Full-suite integration run + regression nets]
  ...
  - Write `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/outcome/5-integration-outcome.md` with the full matrix of commands + results.
...
- [x] 5.2 [Static locks & INV-U4 grep-lock suite]
  ...
  - Final ledger gate: `grep -c "❌\|⚠️" ai/plans/sprint_3/account-soft-delete-governance-usersis-d/deferred-items.md` = 0 (all forward items remain resolved-pointer rows D1-D7).
  - Instruction files: `.agents/instructions/tests.instructions.md`, `.agents/instructions/backend.instructions.md`.
...
> Launch the four review wave agents IN PARALLEL over the full changeset; each writes its finding set into `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/outcome/`. Resolve EVERY finding (fix or ledger-justified rejection) before Phase 7.
...
- [x] 7.4 [Outcome synthesis & final gates]
  - Write `ai/plans/sprint_3/account-soft-delete-governance-usersis-d/outcome/7-completion-outcome.md`: synthesis of ALL task outcomes; final verification table (every plan.md Verification Anchor 1-12 with command + result evidence); baseline diff = 0 attestation; zero-drift `git diff` attestation; codegen artifacts committed attestation; journey green attestation; final `grep -c "❌\|⚠️" ai/plans/sprint_3/account-soft-delete-governance-usersis-d/deferred-items.md` = 0 proof.
