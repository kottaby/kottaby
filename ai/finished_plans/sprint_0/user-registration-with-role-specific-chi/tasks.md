...
1. **Pre-Execution:** Read all outcome files in `ai/plans/user-registration/outcome/` before ANY task.
...
5. **Outcome Documentation:** Write `ai/plans/user-registration/outcome/<task-id>-outcome.md` after each task.
...
- [x] 0.1 Record baseline error counts (`tsgo`, `biome`, `lint-service`) and initialize `ai/plans/user-registration/deferred-items.md` (from the template). _Requirements: REQ-001_
...
- [x] 6.2 Deferred-items gate: `grep -c "❌\|⚠️" ai/plans/user-registration/deferred-items.md` must be 0 before Phase 7.
...
- [x] 7.2 Write final outcome synthesis `ai/plans/user-registration/outcome/plan-completion-outcome.md` (baseline vs final counts, review waves, deferred items resolved, carry-over notes for the Recitation Selection on Registration ticket).
