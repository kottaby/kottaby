...
**Business value:** The gateway skeleton is the last Sprint-0 shared-foundation piece (ROADMAP M0 gate: "CI/CD green" + "all streams can register users, authenticate, and access role-specific endpoints"). It converts the Shared Error Handling & Response Contracts ticket's error contract from a library into a boundary guarantee, prevents per-stream gateway drift across 16+ Sprint 1–3 tickets, and provides the load balancer/CI probe required before any production traffic.
