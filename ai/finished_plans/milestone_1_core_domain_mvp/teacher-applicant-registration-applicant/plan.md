...
> **Plan of record:** `ai/plans/teacher-applicant-registration/`
...
4. Test suites: logic tests, service tests, GraphQL integration tests (via `setupTestServerLifecycle` + `testClient`) — all green (`bun run test:db`, `bun run test:services`, `bun run test:graphql` as applicable), 100% statement/branch on new logic, `bun run test/scripts/run-test.ts` used for DB-bound tests.
