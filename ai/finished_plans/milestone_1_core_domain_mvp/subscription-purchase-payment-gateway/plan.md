...
- **Ticket**: Subscription Purchase via Payment Gateway (Milestone 1, 5 pts) — `docs/planning/TICKETS.md:449-493`
- **Version**: 1.0 · **Date**: 2026-09-06
- **Related Documents**: `docs/billing/plan-catalog.md` (the Plan Catalog CRUD (Admin Only) ticket), `docs/IDEMPOTENCY.md`, `docs/notifications/realtime-engine.md`, `docs/graphql/error-handling-contract.md`, `docs/specs/state-machine-invariants.md`, `docs/specs/open-decisions-and-gaps.md`, `docs/planning/MILESTONE_PLAN.md`

## 1. System Overview & Architecture

### 1.1 What this is
A GraphQL-driven purchase flow backed by a provider-agnostic payment-gateway port. Milestone 1 ships the MOCK provider (`docs/planning/MILESTONE_PLAN.md:161`): the full domain flow (purchase → pending pair → signature-verified webhook → guarded atomic activation → lane credit + notification) is real; only the "gateway" is simulated. Milestone 2 swaps in a real adapter with zero domain/service changes.
...
|---|---|---|---|
| D1 | Mock provider behind `PaymentGatewayPort` (`backend/types/billing/payment-gateway.types.ts` types + `backend/services/billing/payment-gateway/` runtime) | MILESTONE_PLAN:161 mandates mock-for-dev; port + factory mirrors meeting-provider doctrine | Hardcoding a real SDK (no credentials exist), inline "if mock" branches |
...
| D8 | NO student purchase UI; admin plan form gains lane select only | Milestone-2 real-gateway UX; mutation is the Milestone-1 contract | Placeholder checkout page (throwaway work, false UX signal) |
...
- **Explicit no-purchase-UI ruling:** REQ-064; the real-gateway Milestone-2 ticket owns the purchase funnel.
