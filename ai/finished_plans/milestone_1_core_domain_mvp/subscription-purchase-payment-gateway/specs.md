...
- **Ticket Reference**: `docs/planning/TICKETS.md:449-493` (this ticket, Milestone 1, 5 pts, Blocked By the Plan Catalog CRUD (Admin Only) ticket — done in `ai/finished_plans/milestone_1_core_domain_mvp/plan-catalog-crud-admin-only/`)
- **Target Directory**: this directory (under `ai/finished_plans/milestone_1_core_domain_mvp/`)
...
- **OUT**: Real gateway SDK integration (Milestone 2 per `MILESTONE_PLAN.md:161`); segregated crediting refinement & reviews-lane semantics; expiry job & balance zeroing; admin extend/renew/cancel; refund flows; student-facing purchase UI (needs real checkout UX — see REQ-064 ruling); teacher wallet side (existing, untouched).
...
| Purchase-time re-validation contract (INV-PC1/REQ-044) | Documented forward contract, unimplemented | `ai/finished_plans/milestone_1_core_domain_mvp/plan-catalog-crud-admin-only/specs.md:83` |
...
- Provider-agnostic port mandated by `MILESTONE_PLAN.md:161` (mock now, real gateway Milestone 2).
...
| Mock provider | Milestone-1 stand-in implementing the port with deterministic references (`mock_*`) and no network |
