...
**Plan directory (verbatim — every header, ledger path, outcome path, and self-reference in this file uses exactly this string):** `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11`
**Ticket:** this ticket · Milestone 1 · Dev 3 · 2 SP · Blocked-By the Session Creation & Lifecycle ticket (shipped — `docs/sessions/session-lifecycle.md`)
**Inputs:** `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/specs.md` · `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/plan.md`
...
1. **Pre-Execution outcome knowledge read.** Before starting any task, read every existing file under `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/outcome/` plus `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/deferred-items.md`. Do not re-derive a decision a prior outcome already settled; do not repeat an already-recorded deferral.
...
5. **Outcome documentation.** Every completed task writes `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/outcome/<task-id>-outcome.md` capturing: what shipped, deviations from plan, verification evidence (commands + exit codes), deferred discoveries.
...
- **Artifacts:**
  - `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/outcome/0-baseline-outcome.md` (CREATE)
  - `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/deferred-items.md` (CREATE, initialized verbatim from `.agents/spec-process-guide/templates/deferred-items-template.md`)
- **Work:**
...
- **Verification:** quality loop on the doc not applicable (docs) — instead, cross-link check: every referenced doc path exists; every referenced ticket id matches the milestone backlog.
...
- **Work:**
  ...
  - Finalize `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/deferred-items.md` — status every row.
- **Artifacts:** `outcome/7.2-outcome.md`.
- _Requirements: REQ-081_

### - [x] 7.3 [Outcome synthesis & plan closure]
- **Work:**
  - Write `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11/outcome/FINAL-outcome.md`: requirements trace sweep (REQ-001..REQ-081 → implement/test/doc evidence pointers), baseline-vs-final diagnostic deltas, full command transcript index, journey/wire evidence summary, schema-drift-empty proof, and the deferred-items final state.
...
- **Accept:** plan directory self-references are verbatim `ai/plans/milestone_1_core_domain_mvp/recitation-record-per-session-11` everywhere; no task left `[ ]`; ledger reconciled.
