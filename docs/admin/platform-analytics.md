...
- **DO NOT add per-entity drill-downs, arguments, filters, or server-side exports to this surface.** The zero-argument closed contract is the BOLA/BOPLA posture. The one CSV affordance is CLIENT-LOCAL: the dashboard serializes the snapshot already on screen (`frontend/views/admin/analytics/platform-analytics-csv.ts`) — the GraphQL surface exposes no export argument and no server-side export endpoint. Drill-down/detail/filtered or per-entity server-side exports remain separately-owned forward items. (There is likewise NO LIKE/ILIKE predicate on this surface — so no `escapeLikeWildcards` obligation arises; a future drill-down ticket must re-evaluate that.)
