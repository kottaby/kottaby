...
| **INV-S6** (in-session `is_online=false`) | **Landed** (DEV3-005): `startSession` composes `TeacherRepository.setOnline(teacherId, false, tx)` onto the SAME transaction (in `session-lifecycle.service.ts`); the release composes through `releaseTeacherInSessionLock` (`session-lifecycle.transitions.ts`) into `completeSession`, `cancelSession`-from-`started`, and `resolveSessionDispute`-from-`started` (`started_at` classifies the pre-state — a pre-start cancel/arbitration releases nothing). The deliberately-offline restore ceiling (manual toggle) is the DEV2-011/012 seam (ledger D2); the toggle's pre-write guard is `assertTeacherNotInActiveSession` (enforcement module). |
| **INV-S7** (report only on `completed`) | **Landed** (DEV3-005 gate): `assertSessionCompletedForReport(sessionId, tx)` in `backend/services/classes/session-lifecycle.enforcement.ts` (re-exported via the services barrel) — the shared gate; denial = the canonical `SESSION_INVALID_TRANSITION` conflict, identical shape for unknown ids (no existence oracle). Full write surface **shipped** (DEV3-006) — `docs/sessions/session-report-homework.md` (write gate + governance re-check + one-report arbiter). |
| **INV-S8** (homework gated on report) | **Landed** (DEV3-005 gate): `assertReportSubmittedForHomework(sessionId, tx)` in the same enforcement module, backed by the minimal `ReportRepository.existsReportForSession`; denial key `homeworkRequiresReport` (en/ar). Full co-creation surface **shipped** (DEV3-006) — same doc (atomic report+homework co-creation). |
...
|---|---|
| **DEV3-005** (INV-S6/S7/S8 enforcement, status history, dispute surface) | **LANDED** (this slice): the transition matrix + gates live in `session-lifecycle.enforcement.ts` (`isSessionTransitionAllowed`, `assertSessionCompletedForReport`, `assertReportSubmittedForHomework`, `assertTeacherNotInActiveSession`), the lock primitive is `TeacherRepository.setOnline`, the release primitive is `releaseTeacherInSessionLock` (`session-lifecycle.transitions.ts`). The full report/homework write surface has also shipped — `docs/sessions/session-report-homework.md`. Remaining DEV3-005 scope (the `session_status_history` table) is the DEV3-005 ledger's D3 (owned by DEV3-013 and the admin audit stream) — extend the guarded primitives, never duplicate or fork them. |
| **DEV3-006** (reports) | **Implementation shipped** — `docs/sessions/session-report-homework.md` (plan of record `ai/plans/sprint_1/dev3-006-session-report-homework-infrastructure/`), consuming the landed gates `assertSessionCompletedForReport` / `assertReportSubmittedForHomework` from `session-lifecycle.enforcement.ts` (participation checks stay its own, before the gate). Reports hang off `session_id` (C.4 removed the redundant teacher FK). One recitation per session (C.5) — write it via DEV3-007's surface, never from the lifecycle (obligations: `docs/sessions/recitation-record.md` §8). |
| **DEV3-007** (recitation record) | **DELIVERED.** The write-once, one-record-per-session recitation row (C.5). Write and read it ONLY through `RecitationRecordService` / the `sessionRecitation` surface per `docs/sessions/recitation-record.md` — import-by-reference: never a second writer, never a direct table read. |
