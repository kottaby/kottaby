...
| **DEV3-006** (reports) | INV-S7 gating is DEV3-005's; reports hang off `session_id` (C.4 removed the redundant teacher FK). One recitation per session (C.5) — write it via DEV3-007's surface, never from the lifecycle (obligations: `docs/sessions/recitation-record.md` §8). |
