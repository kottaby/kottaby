...
| `app/api/payments/webhook/route.ts` | Gateway callback surface (registry classification `provider-ack-exempt`). Every enabled-path reply IS enveloped — success/unknown-reference/quarantine acks `{ data: { processed, replayed? }, requestId }`; signature rejections 401 and parse/oversize rejections 400 through `apiErrorResponse` (verification of the paymob provider's query-signed `hmac` deliveries lives inside its gateway adapter, whose typed rejections ride the same envelope machinery). The exemption covers the route's two bare-404 gates: the disabled kill switch (`PAYMENT_WEBHOOK_ENABLED` not exactly `"true"` — unset or any other value) and the inactive paymob branch — a query-signed delivery while `PAYMENT_GATEWAY_PROVIDER` is not `paymob` answers a BARE 404 before any body byte is read — no envelope, no `code`, no `requestId` | Server-to-server trust boundary: any structured body from a disabled or inactive-branch surface would prove this path exists (existence oracle); the bare 404 keeps it indistinguishable from an unknown path |
