...
Payload contract: `relatedEntityId` = the request row id on every parent-link row; `relatedEntityType` is AUDIENCE-SCOPED (issue #99) — the STUDENT-targeted incoming-request row carries `"parent_link_request"` (the drawer deep-link key that resolves to `/student/link-requests`), while the PARENT-targeted rows carry the refinement values `"parent_link_request_decision"` (confirm/reject outcome) and `"parent_link_request_expiry"` (pre-expiry reminder), which deliberately MISS the frontend route map and fall through to the `/notifications` feed — constants in `backend/services/parents/parent-link-request.helpers.ts`. Emitted via the engine's `emitForUser`/publish-receipts contracts — publish-after-commit discipline (engine-local receipts render in the bell inbox; verified live in D8 browser QA, `outcome/4.2-4.3-frontend-views-outcome.md` + deferred-item D8 row).
...
8. **R8 — Notifications:** engine-only writes, recipient-locale copy, audience-scoped `relatedEntityType` (student rows `"parent_link_request"` deep-link; parent decision/expiry rows `"parent_link_request_decision"`/`"parent_link_request_expiry"` fall through to the feed — issue #99), publish-after-commit; cancel and expiry emit NOTHING. Anchors: `backend/services/AGENTS.md` (single-writer rule); `backend/services/parents/parent-link-request.helpers.ts` (related-entity-type constants); REQ-018/REQ-023.
...
12. **R12 — Frontend query discipline.** `useQuery`-only reads, id-first `TypedDocumentNode` documents, `roleDashboardPath` for wrong-role redirects (never bare `/dashboard`). Anchors: `frontend/views/students/link-requests/**`, `frontend/views/parent/handshake/**`; live browser proof D8 (`outcome/4.2-4.3-frontend-views-outcome.md`).
...
- **The dashboard discoverability card.** `frontend/views/students/dashboard/PendingParentLinkRequestsCard.tsx` (+ the pure derivation helper `pending-parent-link-requests.ts`, re-exported through the `frontend/views/students/dashboard/index.ts` barrel; mounted beside `HandshakeCodeCard` in the student branch of the dashboard status slot). Data path: the SHARED `myIncomingParentLinkRequestsQueryDocument` (the same id-first document the decision page consumes — one normalized-cache truth, so the decision page's respond write-back re-renders the card to zero) → `useQuery` → `deriveActionableIncoming(rows, nowMs)`. Render contract: query in flight → skeleton card; query error → ONE localized inline alert + retry; zero actionable rows → `null` (renders nothing); N ≥ 1 → title + count chip + the MOST-RECENT requester line (bidi-isolated) + one CTA. Copy resolves through the `parentLink` namespace's six dashboard-card keys (`dashboardCardTitle`, `dashboardCardCount`, `dashboardCardLatestRequester`, `dashboardCardCta`, `dashboardCardLoading`, `dashboardCardLoadError`; retry reuses the `Common` namespace). The CTA navigates to `/student/link-requests`.
- **The notification deep-link.** A drawer notification row for a parent-link request previously hard-anchored `/notifications` (mark-read only — no route resolution existed). The gap was CLOSED with an entity-type-keyed route resolution in the leaf module `frontend/lib/notification-route-resolution.ts`: `STUDENT_LINK_REQUESTS_ROUTE = "/student/link-requests"` is defined ONCE there; the `relatedEntityType → route` map keys on the backend `NotificationType` enum members (the value domain of the persisted varchar — never bare literals), and `resolveNotificationRoute` falls through UNKNOWN/absent pointers to `/notifications` unchanged — never throws, never interpolates `relatedEntityId` (no IDOR/open-redirect surface). The drawer body consumes the resolver for its row anchors; mark-read-then-close behavior is unchanged.
- **Nav retargeting: NO-OP.** The student nav entry ALREADY targeted the real `/student/link-requests` page (not the `[feature]` catch-all) — zero route change. Its route literal was single-sourced onto the shared constant (nav/card/drawer now consume ONE definition site), and the previously-absent nav pin tests were added: exactly ONE student entry at that route, label resolving in both locales, no other role carries the entry.
- **Verification stack added.** Journey `test/workflows/parents/student-confirmation-of-link.journey.test.ts` — 11 actor-attributed steps (request with ONE deep-linked notification; incoming list with the parent's FULL name; wrong-role + foreign-student denials; foreign ≡ nonexistent request id; governed-student denial; reject leg; confirm leg with sibling-pending expiry; parent-side masked-forever pin; deterministic loser collapse; expiry-instant boundary; notification deep-link data contract) plus a TRUE concurrent two-parent race cell gated to real PostgreSQL (skipped on the PGlite provider; the deterministic loser-collapse semantics are pinned everywhere by its sequential twin). Service-suite regression cells: the governance denial is ordered BEFORE the transaction opens (zero repository invocations past the gate) and committed double-respond idempotency (the second respond answers the constant already-resolved conflict; the parent keeps exactly ONE notification + ONE publish). Wire-tier cells: expired-claim denial (fold-first write), foreign ≡ absent byte-identity, and the input-coercion tier.
- **INV-P1 closure (verified, not asserted).** No link exists without the student's explicit confirmation: the guarded single writer remains the ONLY production writer of `students.parent_id` (§3), the journey's negative probes prove wrong-role/foreign/governed actors can NEVER decide, and the race cells prove exactly one winner per pair — the loser commits nothing. The 7-day window stays bound by the open-decisions record (B.14).

Invariants and open decisions are bound by REFERENCE (never renumbered here): [`docs/specs/state-machine-invariants.md`](../specs/state-machine-invariants.md) (INV-P1) and [`docs/specs/open-decisions-and-gaps.md`](../specs/open-decisions-and-gaps.md) (B.14 — 7-day expiry).
...
- Plan artifacts: `ai/plans/milestone_3_parent_portal_admin_governance/parent-child-link-request-workflow-7-day/` (plan.md, tasks.md, deferred-items.md ledger, outcome/).
