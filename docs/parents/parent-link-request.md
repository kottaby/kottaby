...
- `parent_id`, `student_id` (both `NOT NULL`, FK to actors/students), `handshake_code` snapshot, `status`, `created_at`, `expires_at` (`created_at + 7 days`, computed once at insert and stored verbatim), plus `responded_at`/`responded_by`-class transition bookkeeping.
...
| expiry | — | — | SILENT (D1 forward-pointer owns reminders) |
...
- **D1 (cron sweep):** the sweep PRIMITIVE shipped (`markAllExpiredIfPending` + `sweepExpiredRequests`, tested); the cron stream/trigger is the future cron-stream ticket — it registers the primitive as its job handler and unlocks the silent-expiry UX choreography (§5, D9b) on a schedule instead of on-demand.
...
- **Do NOT notify on cancel or expiry.** Silence is contractual (REQ-018, REQ-024). A "courtesy" notification leaks the child's decision timeline to the parent.
...
- **Shipped in this ticket (DEV1-014):** schema + partial unique arbiter (Task 1.2), enums/types/constants (1.3/1.4), i18n namespaces (1.1), repository layer (2.2), service layer (2.3), GraphQL surface (3.1–3.3), frontend wiring + views (4.1–4.4), journey/wire/chaos/static-lock suites (5.1–5.4), four review waves (6.1–6.4) + ledger gate (6.5), live browser QA (D8), the D1 sweep primitive (repo bulk `markAllExpiredIfPending` + service `sweepExpiredRequests` + 6 delta-tested tiers), and the bidi-isolation polish (D8 note a).
