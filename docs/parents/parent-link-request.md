...
- **D1 (cron sweep):** bulk-materialize lapsed pendings + optional expiry reminders. Owns the silent-expiry UX unlock (§5, D9b).
...
6. **R6 — Expiry is strict-`>`, lazily materialized, and silent.** Reads never write (REQ-015); the boundary instant is expired; zero notifications/audit on expiry (REQ-024). Anchors: chaos cells `:607/:654/:693`; render-time mapping at `backend/services/parents/parent-link-request.service.ts:29`.
