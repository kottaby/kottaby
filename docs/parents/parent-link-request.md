...
- **D1 (cron sweep):** the sweep PRIMITIVE shipped (`markAllExpiredIfPending` + `sweepExpiredRequests`, tested) AND the on-demand ops trigger (`scripts/ops/sweep-expired-link-requests.ts`, `bun run ops:sweep-link-requests`) — the sanctioned manual path today; the cron stream/trigger is the future cron-stream ticket — it registers the primitive as its job handler and unlocks the silent-expiry UX choreography (§5, D9b) on a schedule instead of on-demand.
