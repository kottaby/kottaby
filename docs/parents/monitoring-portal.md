...
Per-student service methods (`getChildProgress`, `listChildSessions`, `listChildReports`, `listChildHomework`) open ONE `withTransaction(outerTx, async tx => { ... })` and run the gate FIRST, before any data read, all inside the same snapshot. The `outerTx` parameter lets a caller-owned transaction (e.g., a test under `runInRollback`) join the unit as a SAVEPOINT — the caller owns the commit boundary. A severance that lands mid-flight cannot extend a returned payload — the gate's snapshot is the same snapshot the data reads see. READ COMMITTED per-statement isolation is sufficient; no `FOR UPDATE` is needed because the surface is read-only and a snapshot-time check is the correct semantics.
