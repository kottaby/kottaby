...
| Wallet repository coverage + CHECK constraints | `TODO(NEW-TEST-ANCHOR)` — wallet.repository test (coverage + `wallet_balance_check` / `wallet_total_earning_check` / `teacher_transaction_amount_check` probes) | DB/repo | Pending finalizer: exact path. |
| Financial immutability trigger tier | `TODO(NEW-TEST-ANCHOR)` — immutability trigger-tier test (UPDATE/DELETE rejection on `teacher_transaction`) | DB/repo | Pending finalizer: exact path. |
| Cross-actor financial journey | `TODO(NEW-TEST-ANCHOR)` — financial-safety-verification journey test | Journey | Pending finalizer: exact path. |
...
|---|---|---|
| Database repository tests | `bun run test:db` | Wallet repository coverage + CHECK-constraint probes; financial immutability trigger-tier tests |
| Backend services tests | `bun run test:services` | Real-PG chaos anchors (already present) remain green under the extended assertions |
| Workflow journey layer | `bun run test:scripts` via `bun run test/scripts/run-test.ts test/workflows` | Cross-actor financial-safety verification journey (concurrent double-spend, cancel→release, dual-confirm→credit, withdrawal drain race, identity recomputation) |
