...
webhook route and the whole verify→activation chain runs against production code. Before live
rollout, prove the wallet phone-placeholder tolerance (`"NA"` billing members) against the Paymob
sandbox — REQ-012's placeholder assumption is a manual-QA item, not a test-pinned one.
