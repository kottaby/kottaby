...
- **Race discipline re-proven on real connections — concurrent double-settle (step 6, `test/workflows/billing/admin-financial-auditing.journey.test.ts:824-865`):** two admins approve the same pending request → exactly one approve wins (the loser receives the localized not-pending conflict), exactly one audit row, and one balance movement — the reserve was already taken at request time, so the winning approve settles the reservation instead of debiting again.
