...
## D. Resolved During Implementation (Free Trial Session Provisioning)
...
> **Spec impact:** Deferred item D2 is RESOLVED — `users.locale` (nullable, `AppLocale`) now exists with the `updateMyLocale` mutation. Per-recipient fan-out routing off that column remains open. See `docs/notifications/realtime-engine.md` §3.3.
