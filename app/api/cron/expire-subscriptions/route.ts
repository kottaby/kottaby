 ...
 * following the documented cron rules the sessions sweep route established:
 *
 ...
 *  - **Envelope** — success `{ data: { expired, lanesZeroed }, requestId }`
 *    through `apiSuccessResponse`; a THROWN sweep failure (e.g. an
 *    invariant breach rolling the cohort back) is caught and masked through
 ...
 * per-(owner, lane) zeroings. Zero row identities cross the wire: only the
 * honest counts.
 */
...
function expireUnauthorizedError(): DomainError {
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (secret === undefined || secret.length === 0 || !bearerSecretMatches(presented, secret)) {
    return apiErrorResponse(expireUnauthorizedError(), { requestId, locale: envelopeLocale });
  }

  // The sweep owns its transaction; a thrown failure (an invariant breach →
  // rolled-back cohort) is masked through the shared envelope — never a raw
  // escape past the handler.
  try {
    const result = await SubscriptionExpiryService.expireDue();
    return apiSuccessResponse({ expired: result.expired, lanesZeroed: result.lanesZeroed }, { requestId });
  } catch (error: unknown) {
