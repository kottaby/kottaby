/**
 * `/api/health` — LB-grade HTTP liveness probe. One of EXACTLY TWO sanctioned
 * health surfaces:
 *
 *  1. GraphQL `_health` query (canonical in-band probe);
 *  2. THIS route (load balancers / external uptime checks that must not
 *     parse GraphQL).
 *
 * Both render the SAME single-producer payload by construction:
 ...
 * appear (source-scanned by the paired test suite).
 *
 * Posture (intentional):
 *  - **No auth** — an LB probe carries no session; the payload discloses no
 *    tenancy identity or secrets (the disclosure scan in the paired suite is
 *    the guarantee).
 ...
 *    documentation). No CORS headers are introduced (same-origin-first
 *    posture documented in docs/graphql/error-handling-contract.md §6).
 *  - **Envelope at birth** — success bodies ride the shared API envelope via
 *    `apiSuccessResponse` (`{ data, requestId }`); correlation resolves ONCE
 *    through `resolveRequestId(request.headers)` (single mint source),
