...
for f in "${FILES[@]}"; do
  ...
  if [ "${#batch_files[@]}" -ge "$BATCH" ]; then
    ...
    if ! bun run scripts/lint-service.ts --type-aware "${args[@]}" --id qg-shard > "$TMP_DIR/lint-shard-$i.log" 2>&1; then
      echo "BATCH $i FAILED:"; tail -30 "$TMP_DIR/lint-shard-$i.log"; fail=1; break
    fi
    tail -2 "$TMP_DIR/lint-shard-$i.log"
    batch_files=()
  fi
done
if [ "${#batch_files[@]}" -gt 0 ] && [ "$fail" -eq 0 ]; then
  ...
  if ! bun run scripts/lint-service.ts --type-aware "${args[@]}" --id qg-shard > "$TMP_DIR/lint-shard-$i.log" 2>&1; then
    echo "BATCH $i FAILED:"; tail -30 "$TMP_DIR/lint-shard-$i.log"; fail=1
  else
    tail -2 "$TMP_DIR/lint-shard-$i.log"
  fi
fi
