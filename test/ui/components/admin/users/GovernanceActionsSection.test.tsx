/**
 * GovernanceActionsSection — component suite ENTRY (runner target).
 *
 ...
 * node_modules CJS dependencies BEFORE the file's own static import statements
 * run. A test file that statically imports `@testing-library/react` therefore
 * first evaluates `react-dom` — and if the Happy-DOM global registration only
 ...
 * permanently false → controlled `onChange` dead). The fix is structural: this
 * entry file imports ONLY the local preload chain (test-env → happydom →
 * translation-preload → next-dynamic-mock) — nothing from node_modules — via
 * SEQUENTIAL TOP-LEVEL AWAIT imports; the suite (and with it react-dom) is
 * then loaded via a final top-level `await import(...)`. Every preload is
...
await import("./GovernanceActionsSection.suite");
