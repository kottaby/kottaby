/**
 * TeacherWalletContainer — component suite ENTRY (runner target).
 *
 * Happy DOM + Apollo `MockedProvider` tier (`test/ui/components/teachers/
 * teacher-wallet`, mirroring the teacher sessions suite structure exactly):
 * the suite BODY lives in the sibling `TeacherWalletContainer.suite.tsx` —
 * ONE render case per branch of the teacher wallet visual state matrix,
 * driven across BOTH locales (loading skeleton · FORBIDDEN fallback ·
 * generic error · zeroed fresh-teacher wallet · populated balance cards +
 * ledger · withdrawal dialog open/dismiss · client validation gate ·
 * WALLET_INSUFFICIENT_FUNDS error arm · copy contract pin).
 *
...
await import("./TeacherWalletContainer.suite");
