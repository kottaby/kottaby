/**
 * MySubscriptionsContainer — component suite ENTRY (runner target).
 *
 * Happy DOM + Apollo `MockedProvider` tier (`test/ui/components/student`,
 * mirroring the `StudentSessionsContainer` suite): the suite BODY lives in
 * the sibling `MySubscriptionsContainer.suite.tsx` — ONE render case per
 * branch of the my-subscriptions visual state matrix, driven across BOTH
 * locales (loading skeleton · FORBIDDEN fallback · empty page · populated
 * rows with the failed-payment chip + guidance · copy contract pin).
 *
 ...
 * body runs with NO `document` at all. The fix here is structural: this
 * entry file imports ONLY the local preload chain via SEQUENTIAL TOP-LEVEL
 * AWAIT imports, so the Happy-DOM document is registered first; the suite
 * (and with it react-dom) is then loaded via a final top-level
 * `await import(...)`, guaranteeing react-dom evaluates with
 * `canUseDOM === true` under the plain single-file runner AND the official
...
await import("./MySubscriptionsContainer.suite");
