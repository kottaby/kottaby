/**
 * PaymentResultContainer — component suite ENTRY (runner target).
 *
 * Happy DOM + Apollo `MockedProvider` tier (`test/ui/components/student`,
 * mirroring the plans-catalog and my-subscriptions suite structure
 * exactly): the suite BODY lives in the sibling
 * `PaymentResultContainer.suite.tsx` — ONE render case per branch of the
 * payment-result visual state matrix, driven across BOTH locales
 * (checking arm · FORBIDDEN fallback · generic error · success arm ·
 * pending arm · decline-hint failed arm · zero-trust forged-hint arms ·
 * retry/view CTA navigation · copy contract pin).
 *
...
await import("./PaymentResultContainer.suite");
