/**
 * PlansCatalogContainer — component suite ENTRY (runner target).
 *
 * Happy DOM + Apollo `MockedProvider` tier (`test/ui/components/student`,
 * mirroring the student-sessions and teacher-wallet suite structure
 * exactly): the suite BODY lives in the sibling
 * `PlansCatalogContainer.suite.tsx` — ONE render case per branch of the
 * plan-catalog visual state matrix, driven across BOTH locales (loading
 * skeleton · FORBIDDEN fallback · generic error · empty catalog ·
 * populated card grid · confirm-dialog open/dismiss · idempotency-key
 * pin · redirect/completed/failed purchase arms · copy contract pin).
 *
 * WHY this file exists (two-phase bootstrap) — identical root cause to the
 * sibling suites: Bun evaluates an entry test file's node_modules CJS
 ...
 * `document` at all. The fix is structural: this entry file imports ONLY
 * the local preload chain (test-env → happydom → translation-preload →
 * next-dynamic-mock) via SEQUENTIAL TOP-LEVEL AWAIT imports; the suite
 * (and with it react-dom) is then loaded via a final top-level
 * `await import(...)`. Every preload is idempotent.
 */
...
await import("./PlansCatalogContainer.suite");
