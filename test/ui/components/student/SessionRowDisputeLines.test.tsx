/**
 * SessionRowDisputeLines — component suite ENTRY (runner target).
 *
 * Happy DOM + Apollo `MockedProvider` tier (`test/ui/components/student`,
 * same two-phase bootstrap as the sibling `StudentSessionsContainer` and
 * `StudentPostConfirmationDispute` entries): the suite BODY lives in the
 * sibling `SessionRowDisputeLines.suite.tsx` — the participant-side
 * arbitration-transparency lines over the shared `SessionRow`.
 *
 ...
 * body runs with NO `document` at all. The sequential top-level-await
 * preload chain below registers the Happy-DOM document FIRST; the suite
...
await import("./SessionRowDisputeLines.suite");
