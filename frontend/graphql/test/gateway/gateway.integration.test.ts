...
describe("Gateway integration matrix (REQ-071)", () => {
  ...
  test.skip("(f) authenticated-but-forbidden role-gated op → extensions.code = FORBIDDEN", () => {
  ...
  test.skip("(g) synthetic raw non-DomainError throw → masked INTERNAL_SERVER_ERROR; no stack/SQL/env/path leakage", () => {
  ...
  test.skip("(i) login happy path → three Set-Cookie headers (session_id, refresh_token, access_token)", () => {
  ...
  test.skip("(X-Idempotency-Key) header → present in resolved context", () => {
  ...
  test.skip("(logout forced-failure) → clearing Set-Cookie headers STILL present", () => {
