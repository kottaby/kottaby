...
// Wire-request auth header builder — pure (module scope per
// consistent-function-scoping; the wrapper body owns only hooks + suites).
...
// Whole-file suite guard (skip-when-pglite convention — see
// test/helpers/skip-when-pglite.ts): every tier rides the live wire through a
// warm dev server whose process must share DB state with this one, which the
// single-connection WASM PGlite shim cannot provide. Under PGlite the suite
// skips wholesale (hooks included); in CI / real-PG environments it runs
// exactly as before.
describeGraphqlSuite("handshake code — live-wire integration matrix", () => {
