/**
 * Paired suite — the evaluations tab's completion-notification deep-link
 * highlight: the `?session=` pointer the detail container threads through
 * `renderTabContent` lands on the evaluation row whose owning session it
 * names, with the SAME match + scroll mechanism the reports tab applies.
 * The evaluations tab re-reads the SAME report rows the reports tab lists,
 * so the identical row-level decision drives both highlight lenses.
 *
 * WHAT THIS LOCKS
 *   1. THE MATCH RULE: an evaluation row is the highlight target exactly
 *      when the pointer is present and equals the row's owning session id —
 *      the one shared decision (`isDeepLinkTargetRow`) every content tab's
 *      row applies, so a single link highlights the same session everywhere.
 *   2. THE THREADING: the body renderer hands the pointer to EVERY mounted
 *      evaluation row untouched — no row is skipped, no pointer is rewritten,
 ...
 *      deepLinkSessionId }`, the row object (notes + score) is passed
 *      through unmodified, and a session id absent from the child's own
 *      rows highlights nothing.
 *
 * FIXTURES: plain literal report rows (technical test data — ids, dates,
 * ratings, notes) + the real `parentMonitoring` locale leaf for the labels
 * object. The React tree is inspected as plain elements — no DOM, no
 * renderer.
 *
 * RUNS VIA (in-sandbox): bun run test/scripts/run-test.ts
 * frontend/views/parent/monitoring/EvaluationsTab.sessionHighlight.test.ts
 */
...
function reportRowFixture(
  ...
  return {
    ...
    sessionStartedAt: "2026-09-18T09:00:00.000Z",
    teacherNotes: rating === null ? "" : "Recited Surah Al-Fatiha with clear tajweed",
...
function rowElementsOf(node: ReactNode): readonly ReactElement[] {
  return allElementsOf(node).filter(element => element.type === EvaluationRow);
}
...
describe("isDeepLinkTargetRow — the shared highlight match rule", () => {
  ...
  test("the decision reads only the session id — rated and unrated rows decide identically", () => {
    const ratedRow = reportRowFixture("ev-rated", LINKED_SESSION, 5);
    const unratedRow = reportRowFixture("ev-unrated", LINKED_SESSION, null);
    expect(isDeepLinkTargetRow(LINKED_SESSION, ratedRow.sessionId)).toBe(true);
    expect(isDeepLinkTargetRow(LINKED_SESSION, unratedRow.sessionId)).toBe(true);
    expect(isDeepLinkTargetRow(FOREIGN_SESSION, ratedRow.sessionId)).toBe(
      isDeepLinkTargetRow(FOREIGN_SESSION, unratedRow.sessionId)
    );
  });

  test("rapid pointer alternation — every decision is recomputed from the current pointer", () => {
    const row = reportRowFixture("ev-1", LINKED_SESSION, 5);
...
// ===========================================================================
describe("renderEvaluationsBody — the pointer threads to every mounted evaluation row", () => {
  const rows = [
    reportRowFixture("ev-1", OTHER_SESSION, 4),
    reportRowFixture("ev-2", LINKED_SESSION, 5),
    reportRowFixture("ev-3", THIRD_SESSION, null),
  ];
  ...
  test("with the deep-link pointer — exactly the named session's row is the target", () => {
    ...
    expect(rowSessionIdOf(requireRowElement(targeted, "the linked session's evaluation row should be mounted"))).toBe(
      LINKED_SESSION
    );
  });
  ...
  test("the row prop surface stays closed — the pointer is the ONLY deep-link prop", () => {
    const rowElements = rowElementsOf(renderedBody(rows, LINKED_SESSION));
    const element = requireRowElement(rowElements, "evaluation rows should be mounted");
    expect(sortedKeys(propKeysOf(element))).toEqual(sortedKeys(["deepLinkSessionId", "labels", "locale", "row"]));
  });

  test("rows pass through unmodified — notes and score reach the row untouched", () => {
    const rowElements = rowElementsOf(renderedBody(rows, LINKED_SESSION));
    expect(rowElements).toHaveLength(rows.length);
    for (const element of rowElements) {
...
describe("pending data and re-renders — no stale highlight surface", () => {
  test("while the tab's read is pending only the skeleton mounts — no rows, no scroll, no crash", () => {
    ...
    expect(elements.some(element => element.type === EvaluationRow)).toBe(false);
    expect(elements.some(element => element.type === EvaluationsSkeleton)).toBe(true);
  });

  test("every body render carries exactly its own render's pointer — no stale pointer", () => {
    const rows = [reportRowFixture("ev-1", LINKED_SESSION, 5)];
  ...
  test("a remount without data mounts no rows — the previous render's rows do not leak", () => {
    ...
    expect(allElementsOf(remounted).some(element => element.type === EvaluationsSkeleton)).toBe(true);
  });
});
