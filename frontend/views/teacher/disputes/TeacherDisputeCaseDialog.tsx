...
import type { TeacherDisputeCaseQuery } from "@/frontend/graphql/generated/gql/graphql";
...
/**
 * TeacherDisputeCaseDialog — the session's OWN teacher's case read
 * (the teacher sessions surface, "Case details" on any row that carries
 * dispute history): a single stateful `teacherDisputeCase` query renders
 * the participant-side bundle — the session detail (escrow class, fee,
 * dispute moment + the FULL filed reason), the arbitration decision
 * (outcome + the FULL note + the resolved moment, or the honest pending
 * line while the case awaits arbitration) and the participant-owned
 * artifacts (the teacher's own report with their rating, the homework
 * row, the recitation record) with the student display name resolved
 * server-side.
 *
 ...
 * placeholder data. The state matrix mirrors the admin case-review dialog:
 * `aria-busy` skeleton → denial fallback (`PermissionDeniedFallback` for
 * the mapping-table denial family — the participant gate is SERVER-owned,
 * this surface carries no role logic) → generic inline alert → the bundle.
 *
 * Surface seam: the escrow chip is imported from the admin disputes view
 * because its vocabulary (`escrowHeldChip` / `escrowConsumedChip`) is the
 * SAME arbitration vocabulary both surfaces speak — duplicating the chip
 * would fork the vocabulary, importing the dialog's own sections would
 * fork the testid surface. The admin-only audit trail is deliberately NOT
 * part of this bundle (the trail read asserts an admin actor server-side).
 *
 ...
 * callbacks, RTL-safe logical composition, ≥44px touch target on the
 * close action.
 */
...
function HomeworkBlock({
  ...
  label: string;
  homework: NonNullable<TeacherDisputeCaseQuery["teacherDisputeCase"]["homework"]>;
...
export function TeacherDisputeCaseDialog({
  ...
  const { data, loading, error } = useQuery(teacherDisputeCaseQueryDocument, {
  ...
  if (loading && data === undefined) {
    ...
    body = <TeacherDisputeCaseLoading />;
  } else if (error) {
    ...
    body =
      action?.kind === "permission-fallback" || action?.kind === "auth-recovery" ? (
        ...
        <Stack data-testid="teacher-dispute-case-error" sx={{ py: 4 }}>
    ...
    const disputeCase: TeacherDisputeCaseQuery["teacherDisputeCase"] = data.teacherDisputeCase;
    ...
    const studentLabel = disputeCase.studentName ?? `#${session.studentId}`;
    ...
    body = (
      <Stack sx={{ gap: 3 }}>
        ...
        <Stack data-testid="teacher-dispute-case-session" sx={{ gap: 1.5 }}>
          <Stack
            sx={{
              gap: 1,
              flexDirection: "row",
          ...
          <Stack sx={{ gap: 1.5, flexDirection: "row", flexWrap: "wrap", alignItems: "baseline" }}>
            ...
            <SessionMetaCell label={t.teacherCaseStudentLabel} value={studentLabel} />
          </Stack>
        </Stack>

        {/* The FULL filed reason — never truncated inside the case dialog. */}
        {session.disputeReason !== null ? (
          <Stack data-testid="teacher-dispute-case-reason" sx={{ gap: 0.5 }}>
        ...
        <CaseSection title={t.teacherCaseResolutionTitle} testId="teacher-dispute-case-resolution">
          {isResolved ? (
            <Stack sx={{ gap: 1.5 }}>
              <SessionMetaCell
            ...
            <Alert severity="info" variant="outlined" data-testid="teacher-dispute-case-pending">
        ...
        <CaseSection title={t.teacherCaseReportTitle} testId="teacher-dispute-case-report">
          {report === null ? (
            ...
            <Stack sx={{ gap: 1.5 }}>
              ...
              <SessionMetaCell label={t.teacherCaseRatingLabel} value={report.studentRatingByTeacher.toString()} />
            </Stack>
          )}
        </CaseSection>

        <CaseSection title={t.caseReviewHomeworkTitle} testId="teacher-dispute-case-homework">
          {homework === null ? (
            <Typography variant="body2" sx={theme => ({ color: theme.palette.text.secondary })}>
        ...
        <CaseSection title={t.caseReviewRecitationTitle} testId="teacher-dispute-case-recitation">
          {recitation === null ? (
            <Typography variant="body2" sx={theme => ({ color: theme.palette.text.secondary })}>
  ...
  return (
    <Dialog
      ...
      maxWidth="md"
      scroll="paper"
      data-testid="teacher-dispute-case-dialog"
      aria-labelledby="teacher-dispute-case-title"
    >
      <DialogTitle id="teacher-dispute-case-title" sx={theme => ({ color: theme.palette.onSurface })}>
        {t.teacherCaseTitle}
      </DialogTitle>
      ...
      <DialogActions sx={{ px: 3, pb: 3 }}>
        <Button
          onClick={onClose}
          data-testid="teacher-dispute-case-close"
...
const CASE_LOADING_KEYS: readonly string[] = [
  "teacher-case-loading-session",
  "teacher-case-loading-decision",
  "teacher-case-loading-report",
];

/** The case dialog's `aria-busy` loading slot — bare skeleton lines. */
function TeacherDisputeCaseLoading(): ReactNode {
  return (
    <Stack aria-busy="true" data-testid="teacher-dispute-case-loading" sx={{ gap: 2, py: 2 }}>
