...
describe("formatApplicantDate — timestamp formatting contract", () => {
  ...
  test("Arabic ('ar') locale renders Arabic-Indic digits and 24-hour UTC timestamp", () => {
    const formatted = formatApplicantDate(TEST_ISO, "ar");

    // "ar" locale formats 2026-08-27 13:00 into Arabic-Indic digits with Arabic date separator
    expect(formatted).toContain("٢٠٢٦");
    expect(formatted).toContain("١٣:٠٠");
  });
...
describe("formatDayMonth — chart axis tick formatting contract", () => {
  ...
  test("Arabic ('ar') day/month formatting strips bidi controls and isolates numeric run", () => {
    ...
    const inner = result.slice(1, -1);
    expect(inner).toContain("٢٧");
    expect(inner).toContain("٠٨");
  });
